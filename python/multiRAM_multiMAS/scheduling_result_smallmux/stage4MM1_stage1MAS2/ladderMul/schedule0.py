...
def solve():
	...
	MUL_MEM = S.Resources('MUL_MEM', num=2)
	...
	T8_mem1 += alt(MUL_MEM)
	S += (T1*MUL[0])-1 < T8_mem1*MUL_MEM[1]
	S += T8_mem1 <= T8
	...
	T9_mem1 += alt(MUL_MEM)
	S += (T1*MUL[0])-1 < T9_mem1*MUL_MEM[1]
	S += T9_mem1 <= T9
	...
	S += (T3*MUL[0])-1 < T10_mem1*MUL_MEM[1]
	S += T10_mem1 <= T10
	...
	T11_mem1 += alt(MUL_MEM)
	S += (T3*MUL[0])-1 < T11_mem1*MUL_MEM[1]
	S += T11_mem1 <= T11
	...
	T12_mem0 += alt(MUL_MEM)
	S += (T4*MUL[0])-1 < T12_mem0*MUL_MEM[0]
	S += T12_mem0 <= T12

	T12_mem1 = S.Task('T12_mem1', length=1, delay_cost=1)
	T12_mem1 += alt(MUL_MEM)
	S += (T2*MUL[0])-1 < T12_mem1*MUL_MEM[1]
	S += T12_mem1 <= T12
	...
	T13_mem0 += alt(MUL_MEM)
	S += (T4*MUL[0])-1 < T13_mem0*MUL_MEM[0]
	S += T13_mem0 <= T13

	T13_mem1 = S.Task('T13_mem1', length=1, delay_cost=1)
	T13_mem1 += alt(MUL_MEM)
	S += (T2*MUL[0])-1 < T13_mem1*MUL_MEM[1]
	S += T13_mem1 <= T13
	...
	T15_mem0 += alt(MUL_MEM)
	S += (T7*MUL[0])-1 < T15_mem0*MUL_MEM[0]
	S += T15_mem0 <= T15

	T15_mem1 = S.Task('T15_mem1', length=1, delay_cost=1)
	T15_mem1 += alt(MUL_MEM)
	S += (T7*MUL[0])-1 < T15_mem1*MUL_MEM[1]
	S += T15_mem1 <= T15
	...
	T16_mem0 += alt(MUL_MEM)
	S += (T1*MUL[0])-1 < T16_mem0*MUL_MEM[0]
	S += T16_mem0 <= T16

	T16_mem1 = S.Task('T16_mem1', length=1, delay_cost=1)
	T16_mem1 += alt(MUL_MEM)
	S += (T8*MUL[0])-1 < T16_mem1*MUL_MEM[1]
	S += T16_mem1 <= T16
	...
	T17_mem0 += alt(MUL_MEM)
	S += (T8*MUL[0])-1 < T17_mem0*MUL_MEM[0]
	S += T17_mem0 <= T17
	...
	T18_mem0 += alt(MUL_MEM)
	S += (T5*MUL[0])-1 < T18_mem0*MUL_MEM[0]
	S += T18_mem0 <= T18

	T18_mem1 = S.Task('T18_mem1', length=1, delay_cost=1)
	T18_mem1 += alt(MUL_MEM)
	S += (T9*MUL[0])-1 < T18_mem1*MUL_MEM[1]
	S += T18_mem1 <= T18
	...
	T19_mem0 += alt(MUL_MEM)
	S += (T5*MUL[0])-1 < T19_mem0*MUL_MEM[0]
	S += T19_mem0 <= T19

	T19_mem1 = S.Task('T19_mem1', length=1, delay_cost=1)
	T19_mem1 += alt(MUL_MEM)
	S += (T9*MUL[0])-1 < T19_mem1*MUL_MEM[1]
	S += T19_mem1 <= T19
	...
	S += (T6*MUL[0])-1 < T21_mem0*MUL_MEM[0]
	S += T21_mem0 <= T21

	T21_mem1 = S.Task('T21_mem1', length=1, delay_cost=1)
	T21_mem1 += alt(MUL_MEM)
	S += (T10*MUL[0])-1 < T21_mem1*MUL_MEM[1]
	S += T21_mem1 <= T21
	...
	T24_mem0 += alt(MUL_MEM)
	S += (T11*MUL[0])-1 < T24_mem0*MUL_MEM[0]
	S += T24_mem0 <= T24
	...
	T25_mem0 += alt(MUL_MEM)
	S += (T20*MUL[0])-1 < T25_mem0*MUL_MEM[0]
	S += T25_mem0 <= T25

	T25_mem1 = S.Task('T25_mem1', length=1, delay_cost=1)
	T25_mem1 += alt(MUL_MEM)
	S += (T20*MUL[0])-1 < T25_mem1*MUL_MEM[1]
	S += T25_mem1 <= T25
	...
	Z2_new_mem1 += alt(MUL_MEM)
	S += (T14*MUL[0])-1 < Z2_new_mem1*MUL_MEM[1]
	...
	X2_new_mem0 += alt(MUL_MEM)
	S += (T22*MUL[0])-1 < X2_new_mem0*MUL_MEM[0]
	S += X2_new_mem0 <= X2_new

	X2_new_mem1 = S.Task('X2_new_mem1', length=1, delay_cost=1)
	X2_new_mem1 += alt(MUL_MEM)
	S += (T24*MUL[0])-1 < X2_new_mem1*MUL_MEM[1]
	...
	X1_new_mem0 += alt(MUL_MEM)
	S += (T23*MUL[0])-1 < X1_new_mem0*MUL_MEM[0]
	S += X1_new_mem0 <= X1_new

	X1_new_mem1 = S.Task('X1_new_mem1', length=1, delay_cost=1)
	X1_new_mem1 += alt(MUL_MEM)
	S += (T17*MUL[0])-1 < X1_new_mem1*MUL_MEM[1]
	...
	Z1_new_mem0 += alt(MUL_MEM)
	S += (T16*MUL[0])-1 < Z1_new_mem0*MUL_MEM[0]
	...
	pic_file_name = "/home/mfukuda/ABE/python/multiRAM_multiMAS/scheduling_result/ladderMul_mul1_4_add2_1/schedule0.png"
	if(S.solution() != []):
