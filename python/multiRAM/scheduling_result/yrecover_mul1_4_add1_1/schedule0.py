...
def solve():
	...
	r2_mem0 += alt(MUL_MEM)
	S += (r1*MUL[0])-1 < r2_mem0*MUL_MEM[0]
	S += r2_mem0 <= r2

	r2_mem1 = S.Task('r2_mem1', length=1, delay_cost=1)
	r2_mem1 += alt(MUL_MEM)
	S += (r1*MUL[0])-1 < r2_mem1*MUL_MEM[1]
	S += r2_mem1 <= r2
	...
	r7_mem0 += alt(MUL_MEM)
	S += (r5*MUL[0])-1 < r7_mem0*MUL_MEM[0]
	S += r7_mem0 <= r7

	r7_mem1 = S.Task('r7_mem1', length=1, delay_cost=1)
	r7_mem1 += alt(MUL_MEM)
	S += (r6*MUL[0])-1 < r7_mem1*MUL_MEM[1]
	S += r7_mem1 <= r7
	...
	r8_mem1 += alt(MUL_MEM)
	S += (r4*MUL[0])-1 < r8_mem1*MUL_MEM[1]
	S += r8_mem1 <= r8
	...
	r10_mem1 += alt(MUL_MEM)
	S += (r4*MUL[0])-1 < r10_mem1*MUL_MEM[1]
	S += r10_mem1 <= r10
	...
	r14_mem1 += alt(MUL_MEM)
	S += (r3*MUL[0])-1 < r14_mem1*MUL_MEM[1]
	S += r14_mem1 <= r14
	...
	r15_mem1 += alt(MUL_MEM)
	S += (r9*MUL[0])-1 < r15_mem1*MUL_MEM[1]
	S += r15_mem1 <= r15
	...
	r16_mem0 += alt(MUL_MEM)
	S += (r11*MUL[0])-1 < r16_mem0*MUL_MEM[0]
	S += r16_mem0 <= r16
	...
	r17_mem0 += alt(MUL_MEM)
	S += (r14*MUL[0])-1 < r17_mem0*MUL_MEM[0]
	S += r17_mem0 <= r17

	r17_mem1 = S.Task('r17_mem1', length=1, delay_cost=1)
	r17_mem1 += alt(MUL_MEM)
	S += (r15*MUL[0])-1 < r17_mem1*MUL_MEM[1]
	S += r17_mem1 <= r17
	...
	PX_new_mem1 += alt(MUL_MEM)
	S += (r12*MUL[0])-1 < PX_new_mem1*MUL_MEM[1]
	...
	Z1_new_mem1 += alt(MUL_MEM)
	S += (r13*MUL[0])-1 < Z1_new_mem1*MUL_MEM[1]
	...
	PY_new_mem1 += alt(MUL_MEM)
	S += (r16*MUL[0])-1 < PY_new_mem1*MUL_MEM[1]
	...
	pic_file_name = "/home/mfukuda/ABE/python/multiRAM/scheduling_result/yrecover_mul1_4_add1_1/schedule0.png"
	if(S.solution() != []):
