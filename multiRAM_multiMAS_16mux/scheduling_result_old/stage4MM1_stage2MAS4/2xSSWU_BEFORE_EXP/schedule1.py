...
def solve():
	...
	xi2t40 = S.Task('xi2t40', length=4, delay_cost=1)
	S += xi2t40 >= 7
	xi2t40 += MM[0]
	...
	xi2t41 = S.Task('xi2t41', length=4, delay_cost=1)
	S += xi2t41 >= 8
	xi2t41 += MM[0]
	...
	negD0 = S.Task('negD0', length=4, delay_cost=1)
	S += negD0 >= 13
	negD0 += MM[0]
	...
	negD1 = S.Task('negD1', length=4, delay_cost=1)
	S += negD1 >= 14
	negD1 += MM[0]
	...
	N0 = S.Task('N0', length=4, delay_cost=1)
	S += N0 >= 16
	N0 += MM[0]
	...
	N1 = S.Task('N1', length=4, delay_cost=1)
	S += N1 >= 17
	N1 += MM[0]
	...
	N20 = S.Task('N20', length=4, delay_cost=1)
	S += N20 >= 20
	N20 += MM[0]
	...
	N21 = S.Task('N21', length=4, delay_cost=1)
	S += N21 >= 21
	N21 += MM[0]
	...
	N30 = S.Task('N30', length=4, delay_cost=1)
	S += N30 >= 24
	N30 += MM[0]
	...
	N31 = S.Task('N31', length=4, delay_cost=1)
	S += N31 >= 25
	N31 += MM[0]
	...
	D20 = S.Task('D20', length=4, delay_cost=1)
	S += D20 >= 32
	D20 += MM[0]
	...
	D21 = S.Task('D21', length=4, delay_cost=1)
	S += D21 >= 35
	D21 += MM[0]
	...
	V0 = S.Task('V0', length=4, delay_cost=1)
	S += V0 >= 36
	V0 += MM[0]

	ND20 = S.Task('ND20', length=4, delay_cost=1)
	S += ND20 >= 37
	ND20 += MM[0]
	...
	ND21 = S.Task('ND21', length=4, delay_cost=1)
	S += ND21 >= 39
	ND21 += MM[0]
	...
	V1 = S.Task('V1', length=4, delay_cost=1)
	S += V1 >= 40
	V1 += MM[0]


	# new tasks
	aND20 = S.Task('aND20', length=4, delay_cost=1)
	aND20 += alt(MM)
	...
	aND21 = S.Task('aND21', length=4, delay_cost=1)
	aND21 += alt(MM)
	...
	bD30 = S.Task('bD30', length=4, delay_cost=1)
	bD30 += alt(MM)
	...
	bD31 = S.Task('bD31', length=4, delay_cost=1)
	bD31 += alt(MM)
	...
	V20 = S.Task('V20', length=4, delay_cost=1)
	V20 += alt(MM)
	...
	V21 = S.Task('V21', length=4, delay_cost=1)
	V21 += alt(MM)
	...
	UV0 = S.Task('UV0', length=4, delay_cost=1)
	UV0 += alt(MM)
	...
	UV1 = S.Task('UV1', length=4, delay_cost=1)
	UV1 += alt(MM)
	...
	UV30 = S.Task('UV30', length=4, delay_cost=1)
	UV30 += alt(MM)
	...
	UV31 = S.Task('UV31', length=4, delay_cost=1)
	UV31 += alt(MM)
	...
	pic_file_name = "/home/mfukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage4MM1_stage2MAS4/2xSSWU_BEFORE_EXP/schedule1.png"
	if(S.solution() != []):
