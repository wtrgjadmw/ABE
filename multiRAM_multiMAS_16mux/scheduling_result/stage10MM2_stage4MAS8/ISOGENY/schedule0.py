...
def solve():
	...
	S += NX1__in*MAS_in[6]<=NX1_*MAS[6]

	S += NX1__in*MAS_in[7]<=NX1_*MAS[7]
	...
	S += NY1__in*MAS_in[6]<=NY1_*MAS[6]

	S += NY1__in*MAS_in[7]<=NY1_*MAS[7]
	...
	S += (NX1_*MAS[6])-1 < NX1_mem0*MAS_MEM[12]
	S += (NX1_*MAS[7])-1 < NX1_mem0*MAS_MEM[14]
	S += NX1_mem0 <= NX1
	...
	S += DX1__in*MAS_in[6]<=DX1_*MAS[6]

	S += DX1__in*MAS_in[7]<=DX1_*MAS[7]
	...
	S += (NY1_*MAS[6])-1 < NY1_mem0*MAS_MEM[12]
	S += (NY1_*MAS[7])-1 < NY1_mem0*MAS_MEM[14]
	S += NY1_mem0 <= NY1
	...
	S += DY1__in*MAS_in[6]<=DY1_*MAS[6]

	S += DY1__in*MAS_in[7]<=DY1_*MAS[7]
	...
	pic_file_name = "/mnt/rose/usr1/fukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage10MM2_stage4MAS8/ISOGENY/schedule0.png"
	if(S.solution() != []):
