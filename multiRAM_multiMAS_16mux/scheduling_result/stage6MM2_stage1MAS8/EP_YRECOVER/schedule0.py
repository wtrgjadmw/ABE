...
def solve():
	...
	# new tasks
	r1 = S.Task('r1', length=6, delay_cost=1)
	r1 += alt(MM)
	...
	r3 = S.Task('r3', length=6, delay_cost=1)
	r3 += alt(MM)
	...
	r4 = S.Task('r4', length=6, delay_cost=1)
	r4 += alt(MM)
	...
	r5 = S.Task('r5', length=6, delay_cost=1)
	r5 += alt(MM)
	...
	r6 = S.Task('r6', length=6, delay_cost=1)
	r6 += alt(MM)
	...
	r12 = S.Task('r12', length=6, delay_cost=1)
	r12 += alt(MM)
	...
	r13 = S.Task('r13', length=6, delay_cost=1)
	r13 += alt(MM)
	...
	r9 = S.Task('r9', length=6, delay_cost=1)
	r9 += alt(MM)
	...
	r11 = S.Task('r11', length=6, delay_cost=1)
	r11 += alt(MM)
	...
	PX_new = S.Task('PX_new', length=6, delay_cost=1)
	PX_new += alt(MM)
	...
	r14 = S.Task('r14', length=6, delay_cost=1)
	r14 += alt(MM)
	...
	Z1_new = S.Task('Z1_new', length=6, delay_cost=1)
	Z1_new += alt(MM)
	...
	r15 = S.Task('r15', length=6, delay_cost=1)
	r15 += alt(MM)
	...
	r16 = S.Task('r16', length=6, delay_cost=1)
	r16 += alt(MM)
	...
	pic_file_name = "/home/mfukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage6MM2_stage1MAS8/EP_YRECOVER/schedule0.png"
	if(S.solution() != []):
