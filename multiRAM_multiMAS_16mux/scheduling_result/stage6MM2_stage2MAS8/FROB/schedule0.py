...
def solve():
	...
	# new tasks
	c10_t0 = S.Task('c10_t0', length=6, delay_cost=1)
	c10_t0 += alt(MM)
	...
	c10_t1 = S.Task('c10_t1', length=6, delay_cost=1)
	c10_t1 += alt(MM)
	...
	c20_t0 = S.Task('c20_t0', length=6, delay_cost=1)
	c20_t0 += alt(MM)
	...
	c20_t1 = S.Task('c20_t1', length=6, delay_cost=1)
	c20_t1 += alt(MM)
	...
	c01_t0 = S.Task('c01_t0', length=6, delay_cost=1)
	c01_t0 += alt(MM)
	...
	c01_t1 = S.Task('c01_t1', length=6, delay_cost=1)
	c01_t1 += alt(MM)
	...
	c11_t0 = S.Task('c11_t0', length=6, delay_cost=1)
	c11_t0 += alt(MM)
	...
	c11_t1 = S.Task('c11_t1', length=6, delay_cost=1)
	c11_t1 += alt(MM)
	...
	c21_t0 = S.Task('c21_t0', length=6, delay_cost=1)
	c21_t0 += alt(MM)
	...
	c21_t1 = S.Task('c21_t1', length=6, delay_cost=1)
	c21_t1 += alt(MM)
	...
	pic_file_name = "/mnt/rose/usr1/fukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage6MM2_stage2MAS8/FROB/schedule0.png"
	if(S.solution() != []):
