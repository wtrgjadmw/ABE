from pyschedule import Scenario, solvers, plotters, alt


def solve():
	horizon = 155
	S = Scenario("schedule1", horizon=horizon)

	# resource
	MM = S.Resources('MM', num=2, size=11)
	MM_in = S.Resources('MM_in', num=2)
	MAS_in = S.Resources('MAS_in', num=8)
	CSEL = S.Resource('CSEL')
	MAS = S.Resources('MAS', num=8, size=3, periods=range(1, horizon))
	MM_MEM = S.Resources('MM_MEM', num=4)
	MAS_MEM = S.Resources('MAS_MEM', num=16)
	MAIN_MEM_w = S.Resource('MAIN_MEM_w', size=1)
	MAIN_MEM_r = S.Resources('MAIN_MEM_r', num=2)

	# result of previous scheduling
	t3_t0_in = S.Task('t3_t0_in', length=1, delay_cost=1)
	S += t3_t0_in >= 0
	t3_t0_in += MM_in[0]

	t3_t0_mem0 = S.Task('t3_t0_mem0', length=1, delay_cost=1)
	S += t3_t0_mem0 >= 0
	t3_t0_mem0 += MAIN_MEM_r[0]

	t3_t0_mem1 = S.Task('t3_t0_mem1', length=1, delay_cost=1)
	S += t3_t0_mem1 >= 0
	t3_t0_mem1 += MAIN_MEM_r[1]

	t3_t0 = S.Task('t3_t0', length=11, delay_cost=1)
	S += t3_t0 >= 1
	t3_t0 += MM[0]

	t7_t3_in = S.Task('t7_t3_in', length=1, delay_cost=1)
	S += t7_t3_in >= 1
	t7_t3_in += MAS_in[1]

	t7_t3_mem0 = S.Task('t7_t3_mem0', length=1, delay_cost=1)
	S += t7_t3_mem0 >= 1
	t7_t3_mem0 += MAIN_MEM_r[0]

	t7_t3_mem1 = S.Task('t7_t3_mem1', length=1, delay_cost=1)
	S += t7_t3_mem1 >= 1
	t7_t3_mem1 += MAIN_MEM_r[1]

	t4_t3_in = S.Task('t4_t3_in', length=1, delay_cost=1)
	S += t4_t3_in >= 2
	t4_t3_in += MM_in[0]

	t4_t3_mem0 = S.Task('t4_t3_mem0', length=1, delay_cost=1)
	S += t4_t3_mem0 >= 2
	t4_t3_mem0 += MAIN_MEM_r[0]

	t4_t3_mem1 = S.Task('t4_t3_mem1', length=1, delay_cost=1)
	S += t4_t3_mem1 >= 2
	t4_t3_mem1 += MAIN_MEM_r[1]

	t7_t3 = S.Task('t7_t3', length=3, delay_cost=1)
	S += t7_t3 >= 2
	t7_t3 += MAS[1]

	t0_t1_in = S.Task('t0_t1_in', length=1, delay_cost=1)
	S += t0_t1_in >= 3
	t0_t1_in += MAS_in[1]

	t0_t1_mem0 = S.Task('t0_t1_mem0', length=1, delay_cost=1)
	S += t0_t1_mem0 >= 3
	t0_t1_mem0 += MAIN_MEM_r[0]

	t0_t1_mem1 = S.Task('t0_t1_mem1', length=1, delay_cost=1)
	S += t0_t1_mem1 >= 3
	t0_t1_mem1 += MAIN_MEM_r[1]

	t4_t3 = S.Task('t4_t3', length=11, delay_cost=1)
	S += t4_t3 >= 3
	t4_t3 += MM[0]

	t0_t1 = S.Task('t0_t1', length=3, delay_cost=1)
	S += t0_t1 >= 4
	t0_t1 += MAS[1]

	t11_in = S.Task('t11_in', length=1, delay_cost=1)
	S += t11_in >= 4
	t11_in += MAS_in[2]

	t11_mem0 = S.Task('t11_mem0', length=1, delay_cost=1)
	S += t11_mem0 >= 4
	t11_mem0 += MAIN_MEM_r[0]

	t11_mem1 = S.Task('t11_mem1', length=1, delay_cost=1)
	S += t11_mem1 >= 4
	t11_mem1 += MAIN_MEM_r[1]

	t11 = S.Task('t11', length=3, delay_cost=1)
	S += t11 >= 5
	t11 += MAS[2]

	t6_t3_in = S.Task('t6_t3_in', length=1, delay_cost=1)
	S += t6_t3_in >= 5
	t6_t3_in += MAS_in[0]

	t6_t3_mem0 = S.Task('t6_t3_mem0', length=1, delay_cost=1)
	S += t6_t3_mem0 >= 5
	t6_t3_mem0 += MAIN_MEM_r[0]

	t6_t3_mem1 = S.Task('t6_t3_mem1', length=1, delay_cost=1)
	S += t6_t3_mem1 >= 5
	t6_t3_mem1 += MAIN_MEM_r[1]

	t6_t3 = S.Task('t6_t3', length=3, delay_cost=1)
	S += t6_t3 >= 6
	t6_t3 += MAS[0]

	t7_t0_in = S.Task('t7_t0_in', length=1, delay_cost=1)
	S += t7_t0_in >= 6
	t7_t0_in += MM_in[0]

	t7_t0_mem0 = S.Task('t7_t0_mem0', length=1, delay_cost=1)
	S += t7_t0_mem0 >= 6
	t7_t0_mem0 += MAIN_MEM_r[0]

	t7_t0_mem1 = S.Task('t7_t0_mem1', length=1, delay_cost=1)
	S += t7_t0_mem1 >= 6
	t7_t0_mem1 += MAIN_MEM_r[1]

	t5_t1_in = S.Task('t5_t1_in', length=1, delay_cost=1)
	S += t5_t1_in >= 7
	t5_t1_in += MM_in[1]

	t5_t1_mem0 = S.Task('t5_t1_mem0', length=1, delay_cost=1)
	S += t5_t1_mem0 >= 7
	t5_t1_mem0 += MAIN_MEM_r[0]

	t5_t1_mem1 = S.Task('t5_t1_mem1', length=1, delay_cost=1)
	S += t5_t1_mem1 >= 7
	t5_t1_mem1 += MAIN_MEM_r[1]

	t7_t0 = S.Task('t7_t0', length=11, delay_cost=1)
	S += t7_t0 >= 7
	t7_t0 += MM[0]

	t4_t0_in = S.Task('t4_t0_in', length=1, delay_cost=1)
	S += t4_t0_in >= 8
	t4_t0_in += MAS_in[3]

	t4_t0_mem0 = S.Task('t4_t0_mem0', length=1, delay_cost=1)
	S += t4_t0_mem0 >= 8
	t4_t0_mem0 += MAIN_MEM_r[0]

	t4_t0_mem1 = S.Task('t4_t0_mem1', length=1, delay_cost=1)
	S += t4_t0_mem1 >= 8
	t4_t0_mem1 += MAIN_MEM_r[1]

	t5_t1 = S.Task('t5_t1', length=11, delay_cost=1)
	S += t5_t1 >= 8
	t5_t1 += MM[1]

	t3_t3_in = S.Task('t3_t3_in', length=1, delay_cost=1)
	S += t3_t3_in >= 9
	t3_t3_in += MAS_in[3]

	t3_t3_mem0 = S.Task('t3_t3_mem0', length=1, delay_cost=1)
	S += t3_t3_mem0 >= 9
	t3_t3_mem0 += MAIN_MEM_r[0]

	t3_t3_mem1 = S.Task('t3_t3_mem1', length=1, delay_cost=1)
	S += t3_t3_mem1 >= 9
	t3_t3_mem1 += MAIN_MEM_r[1]

	t4_t0 = S.Task('t4_t0', length=3, delay_cost=1)
	S += t4_t0 >= 9
	t4_t0 += MAS[3]

	t10_in = S.Task('t10_in', length=1, delay_cost=1)
	S += t10_in >= 10
	t10_in += MAS_in[3]

	t10_mem0 = S.Task('t10_mem0', length=1, delay_cost=1)
	S += t10_mem0 >= 10
	t10_mem0 += MAIN_MEM_r[0]

	t10_mem1 = S.Task('t10_mem1', length=1, delay_cost=1)
	S += t10_mem1 >= 10
	t10_mem1 += MAIN_MEM_r[1]

	t3_t3 = S.Task('t3_t3', length=3, delay_cost=1)
	S += t3_t3 >= 10
	t3_t3 += MAS[3]

	t10 = S.Task('t10', length=3, delay_cost=1)
	S += t10 >= 11
	t10 += MAS[3]

	t6_t1_in = S.Task('t6_t1_in', length=1, delay_cost=1)
	S += t6_t1_in >= 11
	t6_t1_in += MM_in[0]

	t6_t1_mem0 = S.Task('t6_t1_mem0', length=1, delay_cost=1)
	S += t6_t1_mem0 >= 11
	t6_t1_mem0 += MAIN_MEM_r[0]

	t6_t1_mem1 = S.Task('t6_t1_mem1', length=1, delay_cost=1)
	S += t6_t1_mem1 >= 11
	t6_t1_mem1 += MAIN_MEM_r[1]

	t0_t0_in = S.Task('t0_t0_in', length=1, delay_cost=1)
	S += t0_t0_in >= 12
	t0_t0_in += MAS_in[3]

	t0_t0_mem0 = S.Task('t0_t0_mem0', length=1, delay_cost=1)
	S += t0_t0_mem0 >= 12
	t0_t0_mem0 += MAIN_MEM_r[0]

	t0_t0_mem1 = S.Task('t0_t0_mem1', length=1, delay_cost=1)
	S += t0_t0_mem1 >= 12
	t0_t0_mem1 += MAIN_MEM_r[1]

	t6_t1 = S.Task('t6_t1', length=11, delay_cost=1)
	S += t6_t1 >= 12
	t6_t1 += MM[0]

	t0_t0 = S.Task('t0_t0', length=3, delay_cost=1)
	S += t0_t0 >= 13
	t0_t0 += MAS[3]

	t7_t1_in = S.Task('t7_t1_in', length=1, delay_cost=1)
	S += t7_t1_in >= 13
	t7_t1_in += MM_in[1]

	t7_t1_mem0 = S.Task('t7_t1_mem0', length=1, delay_cost=1)
	S += t7_t1_mem0 >= 13
	t7_t1_mem0 += MAIN_MEM_r[0]

	t7_t1_mem1 = S.Task('t7_t1_mem1', length=1, delay_cost=1)
	S += t7_t1_mem1 >= 13
	t7_t1_mem1 += MAIN_MEM_r[1]

	t5_t3_in = S.Task('t5_t3_in', length=1, delay_cost=1)
	S += t5_t3_in >= 14
	t5_t3_in += MAS_in[0]

	t5_t3_mem0 = S.Task('t5_t3_mem0', length=1, delay_cost=1)
	S += t5_t3_mem0 >= 14
	t5_t3_mem0 += MAIN_MEM_r[0]

	t5_t3_mem1 = S.Task('t5_t3_mem1', length=1, delay_cost=1)
	S += t5_t3_mem1 >= 14
	t5_t3_mem1 += MAIN_MEM_r[1]

	t7_t1 = S.Task('t7_t1', length=11, delay_cost=1)
	S += t7_t1 >= 14
	t7_t1 += MM[1]

	t4_t1_in = S.Task('t4_t1_in', length=1, delay_cost=1)
	S += t4_t1_in >= 15
	t4_t1_in += MAS_in[7]

	t4_t1_mem0 = S.Task('t4_t1_mem0', length=1, delay_cost=1)
	S += t4_t1_mem0 >= 15
	t4_t1_mem0 += MAIN_MEM_r[0]

	t4_t1_mem1 = S.Task('t4_t1_mem1', length=1, delay_cost=1)
	S += t4_t1_mem1 >= 15
	t4_t1_mem1 += MAIN_MEM_r[1]

	t5_t3 = S.Task('t5_t3', length=3, delay_cost=1)
	S += t5_t3 >= 15
	t5_t3 += MAS[0]

	t4_t1 = S.Task('t4_t1', length=3, delay_cost=1)
	S += t4_t1 >= 16
	t4_t1 += MAS[7]

	t5_t0_in = S.Task('t5_t0_in', length=1, delay_cost=1)
	S += t5_t0_in >= 16
	t5_t0_in += MM_in[0]

	t5_t0_mem0 = S.Task('t5_t0_mem0', length=1, delay_cost=1)
	S += t5_t0_mem0 >= 16
	t5_t0_mem0 += MAIN_MEM_r[0]

	t5_t0_mem1 = S.Task('t5_t0_mem1', length=1, delay_cost=1)
	S += t5_t0_mem1 >= 16
	t5_t0_mem1 += MAIN_MEM_r[1]

	t3_t2_in = S.Task('t3_t2_in', length=1, delay_cost=1)
	S += t3_t2_in >= 17
	t3_t2_in += MAS_in[4]

	t3_t2_mem0 = S.Task('t3_t2_mem0', length=1, delay_cost=1)
	S += t3_t2_mem0 >= 17
	t3_t2_mem0 += MAIN_MEM_r[0]

	t3_t2_mem1 = S.Task('t3_t2_mem1', length=1, delay_cost=1)
	S += t3_t2_mem1 >= 17
	t3_t2_mem1 += MAIN_MEM_r[1]

	t5_t0 = S.Task('t5_t0', length=11, delay_cost=1)
	S += t5_t0 >= 17
	t5_t0 += MM[0]

	t3_t1_in = S.Task('t3_t1_in', length=1, delay_cost=1)
	S += t3_t1_in >= 18
	t3_t1_in += MM_in[0]

	t3_t1_mem0 = S.Task('t3_t1_mem0', length=1, delay_cost=1)
	S += t3_t1_mem0 >= 18
	t3_t1_mem0 += MAIN_MEM_r[0]

	t3_t1_mem1 = S.Task('t3_t1_mem1', length=1, delay_cost=1)
	S += t3_t1_mem1 >= 18
	t3_t1_mem1 += MAIN_MEM_r[1]

	t3_t2 = S.Task('t3_t2', length=3, delay_cost=1)
	S += t3_t2 >= 18
	t3_t2 += MAS[4]

	t3_t1 = S.Task('t3_t1', length=11, delay_cost=1)
	S += t3_t1 >= 19
	t3_t1 += MM[0]

	t5_t2_in = S.Task('t5_t2_in', length=1, delay_cost=1)
	S += t5_t2_in >= 19
	t5_t2_in += MAS_in[6]

	t5_t2_mem0 = S.Task('t5_t2_mem0', length=1, delay_cost=1)
	S += t5_t2_mem0 >= 19
	t5_t2_mem0 += MAIN_MEM_r[0]

	t5_t2_mem1 = S.Task('t5_t2_mem1', length=1, delay_cost=1)
	S += t5_t2_mem1 >= 19
	t5_t2_mem1 += MAIN_MEM_r[1]

	t5_t2 = S.Task('t5_t2', length=3, delay_cost=1)
	S += t5_t2 >= 20
	t5_t2 += MAS[6]

	t6_t0_in = S.Task('t6_t0_in', length=1, delay_cost=1)
	S += t6_t0_in >= 20
	t6_t0_in += MM_in[0]

	t6_t0_mem0 = S.Task('t6_t0_mem0', length=1, delay_cost=1)
	S += t6_t0_mem0 >= 20
	t6_t0_mem0 += MAIN_MEM_r[0]

	t6_t0_mem1 = S.Task('t6_t0_mem1', length=1, delay_cost=1)
	S += t6_t0_mem1 >= 20
	t6_t0_mem1 += MAIN_MEM_r[1]

	t6_t0 = S.Task('t6_t0', length=11, delay_cost=1)
	S += t6_t0 >= 21
	t6_t0 += MM[0]

	t6_t2_in = S.Task('t6_t2_in', length=1, delay_cost=1)
	S += t6_t2_in >= 21
	t6_t2_in += MAS_in[6]

	t6_t2_mem0 = S.Task('t6_t2_mem0', length=1, delay_cost=1)
	S += t6_t2_mem0 >= 21
	t6_t2_mem0 += MAIN_MEM_r[0]

	t6_t2_mem1 = S.Task('t6_t2_mem1', length=1, delay_cost=1)
	S += t6_t2_mem1 >= 21
	t6_t2_mem1 += MAIN_MEM_r[1]

	t6_t2 = S.Task('t6_t2', length=3, delay_cost=1)
	S += t6_t2 >= 22
	t6_t2 += MAS[6]

	t7_t2_in = S.Task('t7_t2_in', length=1, delay_cost=1)
	S += t7_t2_in >= 22
	t7_t2_in += MAS_in[7]

	t7_t2_mem0 = S.Task('t7_t2_mem0', length=1, delay_cost=1)
	S += t7_t2_mem0 >= 22
	t7_t2_mem0 += MAIN_MEM_r[0]

	t7_t2_mem1 = S.Task('t7_t2_mem1', length=1, delay_cost=1)
	S += t7_t2_mem1 >= 22
	t7_t2_mem1 += MAIN_MEM_r[1]

	t0_t3_in = S.Task('t0_t3_in', length=1, delay_cost=1)
	S += t0_t3_in >= 23
	t0_t3_in += MM_in[1]

	t0_t3_mem0 = S.Task('t0_t3_mem0', length=1, delay_cost=1)
	S += t0_t3_mem0 >= 23
	t0_t3_mem0 += MAIN_MEM_r[0]

	t0_t3_mem1 = S.Task('t0_t3_mem1', length=1, delay_cost=1)
	S += t0_t3_mem1 >= 23
	t0_t3_mem1 += MAIN_MEM_r[1]

	t7_t2 = S.Task('t7_t2', length=3, delay_cost=1)
	S += t7_t2 >= 23
	t7_t2 += MAS[7]

	t0_t3 = S.Task('t0_t3', length=11, delay_cost=1)
	S += t0_t3 >= 24
	t0_t3 += MM[1]


	# new tasks
	t0_t2 = S.Task('t0_t2', length=11, delay_cost=1)
	t0_t2 += alt(MM)
	t0_t2_in = S.Task('t0_t2_in', length=1, delay_cost=1)
	t0_t2_in += alt(MM_in)
	S += t0_t2_in*MM_in[0]<=t0_t2*MM[0]
	S += t0_t2_in*MM_in[1]<=t0_t2*MM[1]
	t0_t2_mem0 = S.Task('t0_t2_mem0', length=1, delay_cost=1)
	t0_t2_mem0 += MAS_MEM[6]
	S += 15 < t0_t2_mem0
	S += t0_t2_mem0 <= t0_t2

	t0_t2_mem1 = S.Task('t0_t2_mem1', length=1, delay_cost=1)
	t0_t2_mem1 += MAS_MEM[3]
	S += 6 < t0_t2_mem1
	S += t0_t2_mem1 <= t0_t2

	t0_t5 = S.Task('t0_t5', length=3, delay_cost=1)
	t0_t5 += alt(MAS)
	t0_t5_in = S.Task('t0_t5_in', length=1, delay_cost=1)
	t0_t5_in += alt(MAS_in)
	S += t0_t5_in*MAS_in[0]<=t0_t5*MAS[0]

	S += t0_t5_in*MAS_in[1]<=t0_t5*MAS[1]

	S += t0_t5_in*MAS_in[2]<=t0_t5*MAS[2]

	S += t0_t5_in*MAS_in[3]<=t0_t5*MAS[3]

	S += t0_t5_in*MAS_in[4]<=t0_t5*MAS[4]

	S += t0_t5_in*MAS_in[5]<=t0_t5*MAS[5]

	S += t0_t5_in*MAS_in[6]<=t0_t5*MAS[6]

	S += t0_t5_in*MAS_in[7]<=t0_t5*MAS[7]

	t0_t5_mem0 = S.Task('t0_t5_mem0', length=1, delay_cost=1)
	t0_t5_mem0 += MM_MEM[2]
	S += 34 < t0_t5_mem0
	S += t0_t5_mem0 <= t0_t5

	t0_t5_mem1 = S.Task('t0_t5_mem1', length=1, delay_cost=1)
	t0_t5_mem1 += MM_MEM[3]
	S += 34 < t0_t5_mem1
	S += t0_t5_mem1 <= t0_t5

	t01 = S.Task('t01', length=3, delay_cost=1)
	t01 += alt(MAS)
	t01_in = S.Task('t01_in', length=1, delay_cost=1)
	t01_in += alt(MAS_in)
	S += t01_in*MAS_in[0]<=t01*MAS[0]

	S += t01_in*MAS_in[1]<=t01*MAS[1]

	S += t01_in*MAS_in[2]<=t01*MAS[2]

	S += t01_in*MAS_in[3]<=t01*MAS[3]

	S += t01_in*MAS_in[4]<=t01*MAS[4]

	S += t01_in*MAS_in[5]<=t01*MAS[5]

	S += t01_in*MAS_in[6]<=t01*MAS[6]

	S += t01_in*MAS_in[7]<=t01*MAS[7]

	t01_mem0 = S.Task('t01_mem0', length=1, delay_cost=1)
	t01_mem0 += MM_MEM[2]
	S += 34 < t01_mem0
	S += t01_mem0 <= t01

	t01_mem1 = S.Task('t01_mem1', length=1, delay_cost=1)
	t01_mem1 += MM_MEM[3]
	S += 34 < t01_mem1
	S += t01_mem1 <= t01

	t20 = S.Task('t20', length=3, delay_cost=1)
	t20 += alt(MAS)
	t20_in = S.Task('t20_in', length=1, delay_cost=1)
	t20_in += alt(MAS_in)
	S += t20_in*MAS_in[0]<=t20*MAS[0]

	S += t20_in*MAS_in[1]<=t20*MAS[1]

	S += t20_in*MAS_in[2]<=t20*MAS[2]

	S += t20_in*MAS_in[3]<=t20*MAS[3]

	S += t20_in*MAS_in[4]<=t20*MAS[4]

	S += t20_in*MAS_in[5]<=t20*MAS[5]

	S += t20_in*MAS_in[6]<=t20*MAS[6]

	S += t20_in*MAS_in[7]<=t20*MAS[7]

	t20_mem0 = S.Task('t20_mem0', length=1, delay_cost=1)
	t20_mem0 += MAS_MEM[6]
	S += 13 < t20_mem0
	S += t20_mem0 <= t20

	t20_mem1 = S.Task('t20_mem1', length=1, delay_cost=1)
	t20_mem1 += MAIN_MEM_r[1]
	S += t20_mem1 <= t20

	t21 = S.Task('t21', length=3, delay_cost=1)
	t21 += alt(MAS)
	t21_in = S.Task('t21_in', length=1, delay_cost=1)
	t21_in += alt(MAS_in)
	S += t21_in*MAS_in[0]<=t21*MAS[0]

	S += t21_in*MAS_in[1]<=t21*MAS[1]

	S += t21_in*MAS_in[2]<=t21*MAS[2]

	S += t21_in*MAS_in[3]<=t21*MAS[3]

	S += t21_in*MAS_in[4]<=t21*MAS[4]

	S += t21_in*MAS_in[5]<=t21*MAS[5]

	S += t21_in*MAS_in[6]<=t21*MAS[6]

	S += t21_in*MAS_in[7]<=t21*MAS[7]

	t21_mem0 = S.Task('t21_mem0', length=1, delay_cost=1)
	t21_mem0 += MAS_MEM[4]
	S += 7 < t21_mem0
	S += t21_mem0 <= t21

	t21_mem1 = S.Task('t21_mem1', length=1, delay_cost=1)
	t21_mem1 += MAIN_MEM_r[1]
	S += t21_mem1 <= t21

	t80 = S.Task('t80', length=3, delay_cost=1)
	t80 += alt(MAS)
	t80_in = S.Task('t80_in', length=1, delay_cost=1)
	t80_in += alt(MAS_in)
	S += t80_in*MAS_in[0]<=t80*MAS[0]

	S += t80_in*MAS_in[1]<=t80*MAS[1]

	S += t80_in*MAS_in[2]<=t80*MAS[2]

	S += t80_in*MAS_in[3]<=t80*MAS[3]

	S += t80_in*MAS_in[4]<=t80*MAS[4]

	S += t80_in*MAS_in[5]<=t80*MAS[5]

	S += t80_in*MAS_in[6]<=t80*MAS[6]

	S += t80_in*MAS_in[7]<=t80*MAS[7]

	t80_mem0 = S.Task('t80_mem0', length=1, delay_cost=1)
	t80_mem0 += MAS_MEM[6]
	S += 13 < t80_mem0
	S += t80_mem0 <= t80

	t80_mem1 = S.Task('t80_mem1', length=1, delay_cost=1)
	t80_mem1 += MAIN_MEM_r[1]
	S += t80_mem1 <= t80

	t81 = S.Task('t81', length=3, delay_cost=1)
	t81 += alt(MAS)
	t81_in = S.Task('t81_in', length=1, delay_cost=1)
	t81_in += alt(MAS_in)
	S += t81_in*MAS_in[0]<=t81*MAS[0]

	S += t81_in*MAS_in[1]<=t81*MAS[1]

	S += t81_in*MAS_in[2]<=t81*MAS[2]

	S += t81_in*MAS_in[3]<=t81*MAS[3]

	S += t81_in*MAS_in[4]<=t81*MAS[4]

	S += t81_in*MAS_in[5]<=t81*MAS[5]

	S += t81_in*MAS_in[6]<=t81*MAS[6]

	S += t81_in*MAS_in[7]<=t81*MAS[7]

	t81_mem0 = S.Task('t81_mem0', length=1, delay_cost=1)
	t81_mem0 += MAS_MEM[4]
	S += 7 < t81_mem0
	S += t81_mem0 <= t81

	t81_mem1 = S.Task('t81_mem1', length=1, delay_cost=1)
	t81_mem1 += MAIN_MEM_r[1]
	S += t81_mem1 <= t81

	t3_t4 = S.Task('t3_t4', length=11, delay_cost=1)
	t3_t4 += alt(MM)
	t3_t4_in = S.Task('t3_t4_in', length=1, delay_cost=1)
	t3_t4_in += alt(MM_in)
	S += t3_t4_in*MM_in[0]<=t3_t4*MM[0]
	S += t3_t4_in*MM_in[1]<=t3_t4*MM[1]
	t3_t4_mem0 = S.Task('t3_t4_mem0', length=1, delay_cost=1)
	t3_t4_mem0 += MAS_MEM[8]
	S += 20 < t3_t4_mem0
	S += t3_t4_mem0 <= t3_t4

	t3_t4_mem1 = S.Task('t3_t4_mem1', length=1, delay_cost=1)
	t3_t4_mem1 += MAS_MEM[7]
	S += 12 < t3_t4_mem1
	S += t3_t4_mem1 <= t3_t4

	t30 = S.Task('t30', length=3, delay_cost=1)
	t30 += alt(MAS)
	t30_in = S.Task('t30_in', length=1, delay_cost=1)
	t30_in += alt(MAS_in)
	S += t30_in*MAS_in[0]<=t30*MAS[0]

	S += t30_in*MAS_in[1]<=t30*MAS[1]

	S += t30_in*MAS_in[2]<=t30*MAS[2]

	S += t30_in*MAS_in[3]<=t30*MAS[3]

	S += t30_in*MAS_in[4]<=t30*MAS[4]

	S += t30_in*MAS_in[5]<=t30*MAS[5]

	S += t30_in*MAS_in[6]<=t30*MAS[6]

	S += t30_in*MAS_in[7]<=t30*MAS[7]

	t30_mem0 = S.Task('t30_mem0', length=1, delay_cost=1)
	t30_mem0 += MM_MEM[0]
	S += 11 < t30_mem0
	S += t30_mem0 <= t30

	t30_mem1 = S.Task('t30_mem1', length=1, delay_cost=1)
	t30_mem1 += MM_MEM[1]
	S += 29 < t30_mem1
	S += t30_mem1 <= t30

	t3_t5 = S.Task('t3_t5', length=3, delay_cost=1)
	t3_t5 += alt(MAS)
	t3_t5_in = S.Task('t3_t5_in', length=1, delay_cost=1)
	t3_t5_in += alt(MAS_in)
	S += t3_t5_in*MAS_in[0]<=t3_t5*MAS[0]

	S += t3_t5_in*MAS_in[1]<=t3_t5*MAS[1]

	S += t3_t5_in*MAS_in[2]<=t3_t5*MAS[2]

	S += t3_t5_in*MAS_in[3]<=t3_t5*MAS[3]

	S += t3_t5_in*MAS_in[4]<=t3_t5*MAS[4]

	S += t3_t5_in*MAS_in[5]<=t3_t5*MAS[5]

	S += t3_t5_in*MAS_in[6]<=t3_t5*MAS[6]

	S += t3_t5_in*MAS_in[7]<=t3_t5*MAS[7]

	t3_t5_mem0 = S.Task('t3_t5_mem0', length=1, delay_cost=1)
	t3_t5_mem0 += MM_MEM[0]
	S += 11 < t3_t5_mem0
	S += t3_t5_mem0 <= t3_t5

	t3_t5_mem1 = S.Task('t3_t5_mem1', length=1, delay_cost=1)
	t3_t5_mem1 += MM_MEM[1]
	S += 29 < t3_t5_mem1
	S += t3_t5_mem1 <= t3_t5

	t4_t2 = S.Task('t4_t2', length=11, delay_cost=1)
	t4_t2 += alt(MM)
	t4_t2_in = S.Task('t4_t2_in', length=1, delay_cost=1)
	t4_t2_in += alt(MM_in)
	S += t4_t2_in*MM_in[0]<=t4_t2*MM[0]
	S += t4_t2_in*MM_in[1]<=t4_t2*MM[1]
	t4_t2_mem0 = S.Task('t4_t2_mem0', length=1, delay_cost=1)
	t4_t2_mem0 += MAS_MEM[6]
	S += 11 < t4_t2_mem0
	S += t4_t2_mem0 <= t4_t2

	t4_t2_mem1 = S.Task('t4_t2_mem1', length=1, delay_cost=1)
	t4_t2_mem1 += MAS_MEM[15]
	S += 18 < t4_t2_mem1
	S += t4_t2_mem1 <= t4_t2

	t4_t5 = S.Task('t4_t5', length=3, delay_cost=1)
	t4_t5 += alt(MAS)
	t4_t5_in = S.Task('t4_t5_in', length=1, delay_cost=1)
	t4_t5_in += alt(MAS_in)
	S += t4_t5_in*MAS_in[0]<=t4_t5*MAS[0]

	S += t4_t5_in*MAS_in[1]<=t4_t5*MAS[1]

	S += t4_t5_in*MAS_in[2]<=t4_t5*MAS[2]

	S += t4_t5_in*MAS_in[3]<=t4_t5*MAS[3]

	S += t4_t5_in*MAS_in[4]<=t4_t5*MAS[4]

	S += t4_t5_in*MAS_in[5]<=t4_t5*MAS[5]

	S += t4_t5_in*MAS_in[6]<=t4_t5*MAS[6]

	S += t4_t5_in*MAS_in[7]<=t4_t5*MAS[7]

	t4_t5_mem0 = S.Task('t4_t5_mem0', length=1, delay_cost=1)
	t4_t5_mem0 += MM_MEM[0]
	S += 13 < t4_t5_mem0
	S += t4_t5_mem0 <= t4_t5

	t4_t5_mem1 = S.Task('t4_t5_mem1', length=1, delay_cost=1)
	t4_t5_mem1 += MM_MEM[1]
	S += 13 < t4_t5_mem1
	S += t4_t5_mem1 <= t4_t5

	t41 = S.Task('t41', length=3, delay_cost=1)
	t41 += alt(MAS)
	t41_in = S.Task('t41_in', length=1, delay_cost=1)
	t41_in += alt(MAS_in)
	S += t41_in*MAS_in[0]<=t41*MAS[0]

	S += t41_in*MAS_in[1]<=t41*MAS[1]

	S += t41_in*MAS_in[2]<=t41*MAS[2]

	S += t41_in*MAS_in[3]<=t41*MAS[3]

	S += t41_in*MAS_in[4]<=t41*MAS[4]

	S += t41_in*MAS_in[5]<=t41*MAS[5]

	S += t41_in*MAS_in[6]<=t41*MAS[6]

	S += t41_in*MAS_in[7]<=t41*MAS[7]

	t41_mem0 = S.Task('t41_mem0', length=1, delay_cost=1)
	t41_mem0 += MM_MEM[0]
	S += 13 < t41_mem0
	S += t41_mem0 <= t41

	t41_mem1 = S.Task('t41_mem1', length=1, delay_cost=1)
	t41_mem1 += MM_MEM[1]
	S += 13 < t41_mem1
	S += t41_mem1 <= t41

	t5_t4 = S.Task('t5_t4', length=11, delay_cost=1)
	t5_t4 += alt(MM)
	t5_t4_in = S.Task('t5_t4_in', length=1, delay_cost=1)
	t5_t4_in += alt(MM_in)
	S += t5_t4_in*MM_in[0]<=t5_t4*MM[0]
	S += t5_t4_in*MM_in[1]<=t5_t4*MM[1]
	t5_t4_mem0 = S.Task('t5_t4_mem0', length=1, delay_cost=1)
	t5_t4_mem0 += MAS_MEM[12]
	S += 22 < t5_t4_mem0
	S += t5_t4_mem0 <= t5_t4

	t5_t4_mem1 = S.Task('t5_t4_mem1', length=1, delay_cost=1)
	t5_t4_mem1 += MAS_MEM[1]
	S += 17 < t5_t4_mem1
	S += t5_t4_mem1 <= t5_t4

	t50 = S.Task('t50', length=3, delay_cost=1)
	t50 += alt(MAS)
	t50_in = S.Task('t50_in', length=1, delay_cost=1)
	t50_in += alt(MAS_in)
	S += t50_in*MAS_in[0]<=t50*MAS[0]

	S += t50_in*MAS_in[1]<=t50*MAS[1]

	S += t50_in*MAS_in[2]<=t50*MAS[2]

	S += t50_in*MAS_in[3]<=t50*MAS[3]

	S += t50_in*MAS_in[4]<=t50*MAS[4]

	S += t50_in*MAS_in[5]<=t50*MAS[5]

	S += t50_in*MAS_in[6]<=t50*MAS[6]

	S += t50_in*MAS_in[7]<=t50*MAS[7]

	t50_mem0 = S.Task('t50_mem0', length=1, delay_cost=1)
	t50_mem0 += MM_MEM[0]
	S += 27 < t50_mem0
	S += t50_mem0 <= t50

	t50_mem1 = S.Task('t50_mem1', length=1, delay_cost=1)
	t50_mem1 += MM_MEM[3]
	S += 18 < t50_mem1
	S += t50_mem1 <= t50

	t5_t5 = S.Task('t5_t5', length=3, delay_cost=1)
	t5_t5 += alt(MAS)
	t5_t5_in = S.Task('t5_t5_in', length=1, delay_cost=1)
	t5_t5_in += alt(MAS_in)
	S += t5_t5_in*MAS_in[0]<=t5_t5*MAS[0]

	S += t5_t5_in*MAS_in[1]<=t5_t5*MAS[1]

	S += t5_t5_in*MAS_in[2]<=t5_t5*MAS[2]

	S += t5_t5_in*MAS_in[3]<=t5_t5*MAS[3]

	S += t5_t5_in*MAS_in[4]<=t5_t5*MAS[4]

	S += t5_t5_in*MAS_in[5]<=t5_t5*MAS[5]

	S += t5_t5_in*MAS_in[6]<=t5_t5*MAS[6]

	S += t5_t5_in*MAS_in[7]<=t5_t5*MAS[7]

	t5_t5_mem0 = S.Task('t5_t5_mem0', length=1, delay_cost=1)
	t5_t5_mem0 += MM_MEM[0]
	S += 27 < t5_t5_mem0
	S += t5_t5_mem0 <= t5_t5

	t5_t5_mem1 = S.Task('t5_t5_mem1', length=1, delay_cost=1)
	t5_t5_mem1 += MM_MEM[3]
	S += 18 < t5_t5_mem1
	S += t5_t5_mem1 <= t5_t5

	t6_t4 = S.Task('t6_t4', length=11, delay_cost=1)
	t6_t4 += alt(MM)
	t6_t4_in = S.Task('t6_t4_in', length=1, delay_cost=1)
	t6_t4_in += alt(MM_in)
	S += t6_t4_in*MM_in[0]<=t6_t4*MM[0]
	S += t6_t4_in*MM_in[1]<=t6_t4*MM[1]
	t6_t4_mem0 = S.Task('t6_t4_mem0', length=1, delay_cost=1)
	t6_t4_mem0 += MAS_MEM[12]
	S += 24 < t6_t4_mem0
	S += t6_t4_mem0 <= t6_t4

	t6_t4_mem1 = S.Task('t6_t4_mem1', length=1, delay_cost=1)
	t6_t4_mem1 += MAS_MEM[1]
	S += 8 < t6_t4_mem1
	S += t6_t4_mem1 <= t6_t4

	t60 = S.Task('t60', length=3, delay_cost=1)
	t60 += alt(MAS)
	t60_in = S.Task('t60_in', length=1, delay_cost=1)
	t60_in += alt(MAS_in)
	S += t60_in*MAS_in[0]<=t60*MAS[0]

	S += t60_in*MAS_in[1]<=t60*MAS[1]

	S += t60_in*MAS_in[2]<=t60*MAS[2]

	S += t60_in*MAS_in[3]<=t60*MAS[3]

	S += t60_in*MAS_in[4]<=t60*MAS[4]

	S += t60_in*MAS_in[5]<=t60*MAS[5]

	S += t60_in*MAS_in[6]<=t60*MAS[6]

	S += t60_in*MAS_in[7]<=t60*MAS[7]

	t60_mem0 = S.Task('t60_mem0', length=1, delay_cost=1)
	t60_mem0 += MM_MEM[0]
	S += 31 < t60_mem0
	S += t60_mem0 <= t60

	t60_mem1 = S.Task('t60_mem1', length=1, delay_cost=1)
	t60_mem1 += MM_MEM[1]
	S += 22 < t60_mem1
	S += t60_mem1 <= t60

	t6_t5 = S.Task('t6_t5', length=3, delay_cost=1)
	t6_t5 += alt(MAS)
	t6_t5_in = S.Task('t6_t5_in', length=1, delay_cost=1)
	t6_t5_in += alt(MAS_in)
	S += t6_t5_in*MAS_in[0]<=t6_t5*MAS[0]

	S += t6_t5_in*MAS_in[1]<=t6_t5*MAS[1]

	S += t6_t5_in*MAS_in[2]<=t6_t5*MAS[2]

	S += t6_t5_in*MAS_in[3]<=t6_t5*MAS[3]

	S += t6_t5_in*MAS_in[4]<=t6_t5*MAS[4]

	S += t6_t5_in*MAS_in[5]<=t6_t5*MAS[5]

	S += t6_t5_in*MAS_in[6]<=t6_t5*MAS[6]

	S += t6_t5_in*MAS_in[7]<=t6_t5*MAS[7]

	t6_t5_mem0 = S.Task('t6_t5_mem0', length=1, delay_cost=1)
	t6_t5_mem0 += MM_MEM[0]
	S += 31 < t6_t5_mem0
	S += t6_t5_mem0 <= t6_t5

	t6_t5_mem1 = S.Task('t6_t5_mem1', length=1, delay_cost=1)
	t6_t5_mem1 += MM_MEM[1]
	S += 22 < t6_t5_mem1
	S += t6_t5_mem1 <= t6_t5

	t7_t4 = S.Task('t7_t4', length=11, delay_cost=1)
	t7_t4 += alt(MM)
	t7_t4_in = S.Task('t7_t4_in', length=1, delay_cost=1)
	t7_t4_in += alt(MM_in)
	S += t7_t4_in*MM_in[0]<=t7_t4*MM[0]
	S += t7_t4_in*MM_in[1]<=t7_t4*MM[1]
	t7_t4_mem0 = S.Task('t7_t4_mem0', length=1, delay_cost=1)
	t7_t4_mem0 += MAS_MEM[14]
	S += 25 < t7_t4_mem0
	S += t7_t4_mem0 <= t7_t4

	t7_t4_mem1 = S.Task('t7_t4_mem1', length=1, delay_cost=1)
	t7_t4_mem1 += MAS_MEM[3]
	S += 4 < t7_t4_mem1
	S += t7_t4_mem1 <= t7_t4

	t70 = S.Task('t70', length=3, delay_cost=1)
	t70 += alt(MAS)
	t70_in = S.Task('t70_in', length=1, delay_cost=1)
	t70_in += alt(MAS_in)
	S += t70_in*MAS_in[0]<=t70*MAS[0]

	S += t70_in*MAS_in[1]<=t70*MAS[1]

	S += t70_in*MAS_in[2]<=t70*MAS[2]

	S += t70_in*MAS_in[3]<=t70*MAS[3]

	S += t70_in*MAS_in[4]<=t70*MAS[4]

	S += t70_in*MAS_in[5]<=t70*MAS[5]

	S += t70_in*MAS_in[6]<=t70*MAS[6]

	S += t70_in*MAS_in[7]<=t70*MAS[7]

	t70_mem0 = S.Task('t70_mem0', length=1, delay_cost=1)
	t70_mem0 += MM_MEM[0]
	S += 17 < t70_mem0
	S += t70_mem0 <= t70

	t70_mem1 = S.Task('t70_mem1', length=1, delay_cost=1)
	t70_mem1 += MM_MEM[3]
	S += 24 < t70_mem1
	S += t70_mem1 <= t70

	t7_t5 = S.Task('t7_t5', length=3, delay_cost=1)
	t7_t5 += alt(MAS)
	t7_t5_in = S.Task('t7_t5_in', length=1, delay_cost=1)
	t7_t5_in += alt(MAS_in)
	S += t7_t5_in*MAS_in[0]<=t7_t5*MAS[0]

	S += t7_t5_in*MAS_in[1]<=t7_t5*MAS[1]

	S += t7_t5_in*MAS_in[2]<=t7_t5*MAS[2]

	S += t7_t5_in*MAS_in[3]<=t7_t5*MAS[3]

	S += t7_t5_in*MAS_in[4]<=t7_t5*MAS[4]

	S += t7_t5_in*MAS_in[5]<=t7_t5*MAS[5]

	S += t7_t5_in*MAS_in[6]<=t7_t5*MAS[6]

	S += t7_t5_in*MAS_in[7]<=t7_t5*MAS[7]

	t7_t5_mem0 = S.Task('t7_t5_mem0', length=1, delay_cost=1)
	t7_t5_mem0 += MM_MEM[0]
	S += 17 < t7_t5_mem0
	S += t7_t5_mem0 <= t7_t5

	t7_t5_mem1 = S.Task('t7_t5_mem1', length=1, delay_cost=1)
	t7_t5_mem1 += MM_MEM[3]
	S += 24 < t7_t5_mem1
	S += t7_t5_mem1 <= t7_t5

	t00 = S.Task('t00', length=3, delay_cost=1)
	t00 += alt(MAS)
	t00_in = S.Task('t00_in', length=1, delay_cost=1)
	t00_in += alt(MAS_in)
	S += t00_in*MAS_in[0]<=t00*MAS[0]

	S += t00_in*MAS_in[1]<=t00*MAS[1]

	S += t00_in*MAS_in[2]<=t00*MAS[2]

	S += t00_in*MAS_in[3]<=t00*MAS[3]

	S += t00_in*MAS_in[4]<=t00*MAS[4]

	S += t00_in*MAS_in[5]<=t00*MAS[5]

	S += t00_in*MAS_in[6]<=t00*MAS[6]

	S += t00_in*MAS_in[7]<=t00*MAS[7]

	t00_mem0 = S.Task('t00_mem0', length=1, delay_cost=1)
	t00_mem0 += alt(MM_MEM)
	S += (t0_t2*MM[0])-1 < t00_mem0*MM_MEM[0]
	S += (t0_t2*MM[1])-1 < t00_mem0*MM_MEM[2]
	S += t00_mem0 <= t00

	t00_mem1 = S.Task('t00_mem1', length=1, delay_cost=1)
	t00_mem1 += alt(MAS_MEM)
	S += (t0_t5*MAS[0])-1 < t00_mem1*MAS_MEM[1]
	S += (t0_t5*MAS[1])-1 < t00_mem1*MAS_MEM[3]
	S += (t0_t5*MAS[2])-1 < t00_mem1*MAS_MEM[5]
	S += (t0_t5*MAS[3])-1 < t00_mem1*MAS_MEM[7]
	S += (t0_t5*MAS[4])-1 < t00_mem1*MAS_MEM[9]
	S += (t0_t5*MAS[5])-1 < t00_mem1*MAS_MEM[11]
	S += (t0_t5*MAS[6])-1 < t00_mem1*MAS_MEM[13]
	S += (t0_t5*MAS[7])-1 < t00_mem1*MAS_MEM[15]
	S += t00_mem1 <= t00

	t9_t0 = S.Task('t9_t0', length=3, delay_cost=1)
	t9_t0 += alt(MAS)
	t9_t0_in = S.Task('t9_t0_in', length=1, delay_cost=1)
	t9_t0_in += alt(MAS_in)
	S += t9_t0_in*MAS_in[0]<=t9_t0*MAS[0]

	S += t9_t0_in*MAS_in[1]<=t9_t0*MAS[1]

	S += t9_t0_in*MAS_in[2]<=t9_t0*MAS[2]

	S += t9_t0_in*MAS_in[3]<=t9_t0*MAS[3]

	S += t9_t0_in*MAS_in[4]<=t9_t0*MAS[4]

	S += t9_t0_in*MAS_in[5]<=t9_t0*MAS[5]

	S += t9_t0_in*MAS_in[6]<=t9_t0*MAS[6]

	S += t9_t0_in*MAS_in[7]<=t9_t0*MAS[7]

	t9_t0_mem0 = S.Task('t9_t0_mem0', length=1, delay_cost=1)
	t9_t0_mem0 += alt(MAS_MEM)
	S += (t80*MAS[0])-1 < t9_t0_mem0*MAS_MEM[0]
	S += (t80*MAS[1])-1 < t9_t0_mem0*MAS_MEM[2]
	S += (t80*MAS[2])-1 < t9_t0_mem0*MAS_MEM[4]
	S += (t80*MAS[3])-1 < t9_t0_mem0*MAS_MEM[6]
	S += (t80*MAS[4])-1 < t9_t0_mem0*MAS_MEM[8]
	S += (t80*MAS[5])-1 < t9_t0_mem0*MAS_MEM[10]
	S += (t80*MAS[6])-1 < t9_t0_mem0*MAS_MEM[12]
	S += (t80*MAS[7])-1 < t9_t0_mem0*MAS_MEM[14]
	S += t9_t0_mem0 <= t9_t0

	t9_t0_mem1 = S.Task('t9_t0_mem1', length=1, delay_cost=1)
	t9_t0_mem1 += alt(MAS_MEM)
	S += (t81*MAS[0])-1 < t9_t0_mem1*MAS_MEM[1]
	S += (t81*MAS[1])-1 < t9_t0_mem1*MAS_MEM[3]
	S += (t81*MAS[2])-1 < t9_t0_mem1*MAS_MEM[5]
	S += (t81*MAS[3])-1 < t9_t0_mem1*MAS_MEM[7]
	S += (t81*MAS[4])-1 < t9_t0_mem1*MAS_MEM[9]
	S += (t81*MAS[5])-1 < t9_t0_mem1*MAS_MEM[11]
	S += (t81*MAS[6])-1 < t9_t0_mem1*MAS_MEM[13]
	S += (t81*MAS[7])-1 < t9_t0_mem1*MAS_MEM[15]
	S += t9_t0_mem1 <= t9_t0

	t9_t1 = S.Task('t9_t1', length=3, delay_cost=1)
	t9_t1 += alt(MAS)
	t9_t1_in = S.Task('t9_t1_in', length=1, delay_cost=1)
	t9_t1_in += alt(MAS_in)
	S += t9_t1_in*MAS_in[0]<=t9_t1*MAS[0]

	S += t9_t1_in*MAS_in[1]<=t9_t1*MAS[1]

	S += t9_t1_in*MAS_in[2]<=t9_t1*MAS[2]

	S += t9_t1_in*MAS_in[3]<=t9_t1*MAS[3]

	S += t9_t1_in*MAS_in[4]<=t9_t1*MAS[4]

	S += t9_t1_in*MAS_in[5]<=t9_t1*MAS[5]

	S += t9_t1_in*MAS_in[6]<=t9_t1*MAS[6]

	S += t9_t1_in*MAS_in[7]<=t9_t1*MAS[7]

	t9_t1_mem0 = S.Task('t9_t1_mem0', length=1, delay_cost=1)
	t9_t1_mem0 += alt(MAS_MEM)
	S += (t80*MAS[0])-1 < t9_t1_mem0*MAS_MEM[0]
	S += (t80*MAS[1])-1 < t9_t1_mem0*MAS_MEM[2]
	S += (t80*MAS[2])-1 < t9_t1_mem0*MAS_MEM[4]
	S += (t80*MAS[3])-1 < t9_t1_mem0*MAS_MEM[6]
	S += (t80*MAS[4])-1 < t9_t1_mem0*MAS_MEM[8]
	S += (t80*MAS[5])-1 < t9_t1_mem0*MAS_MEM[10]
	S += (t80*MAS[6])-1 < t9_t1_mem0*MAS_MEM[12]
	S += (t80*MAS[7])-1 < t9_t1_mem0*MAS_MEM[14]
	S += t9_t1_mem0 <= t9_t1

	t9_t1_mem1 = S.Task('t9_t1_mem1', length=1, delay_cost=1)
	t9_t1_mem1 += alt(MAS_MEM)
	S += (t81*MAS[0])-1 < t9_t1_mem1*MAS_MEM[1]
	S += (t81*MAS[1])-1 < t9_t1_mem1*MAS_MEM[3]
	S += (t81*MAS[2])-1 < t9_t1_mem1*MAS_MEM[5]
	S += (t81*MAS[3])-1 < t9_t1_mem1*MAS_MEM[7]
	S += (t81*MAS[4])-1 < t9_t1_mem1*MAS_MEM[9]
	S += (t81*MAS[5])-1 < t9_t1_mem1*MAS_MEM[11]
	S += (t81*MAS[6])-1 < t9_t1_mem1*MAS_MEM[13]
	S += (t81*MAS[7])-1 < t9_t1_mem1*MAS_MEM[15]
	S += t9_t1_mem1 <= t9_t1

	t9_t3 = S.Task('t9_t3', length=11, delay_cost=1)
	t9_t3 += alt(MM)
	t9_t3_in = S.Task('t9_t3_in', length=1, delay_cost=1)
	t9_t3_in += alt(MM_in)
	S += t9_t3_in*MM_in[0]<=t9_t3*MM[0]
	S += t9_t3_in*MM_in[1]<=t9_t3*MM[1]
	t9_t3_mem0 = S.Task('t9_t3_mem0', length=1, delay_cost=1)
	t9_t3_mem0 += alt(MAS_MEM)
	S += (t80*MAS[0])-1 < t9_t3_mem0*MAS_MEM[0]
	S += (t80*MAS[1])-1 < t9_t3_mem0*MAS_MEM[2]
	S += (t80*MAS[2])-1 < t9_t3_mem0*MAS_MEM[4]
	S += (t80*MAS[3])-1 < t9_t3_mem0*MAS_MEM[6]
	S += (t80*MAS[4])-1 < t9_t3_mem0*MAS_MEM[8]
	S += (t80*MAS[5])-1 < t9_t3_mem0*MAS_MEM[10]
	S += (t80*MAS[6])-1 < t9_t3_mem0*MAS_MEM[12]
	S += (t80*MAS[7])-1 < t9_t3_mem0*MAS_MEM[14]
	S += t9_t3_mem0 <= t9_t3

	t9_t3_mem1 = S.Task('t9_t3_mem1', length=1, delay_cost=1)
	t9_t3_mem1 += alt(MAS_MEM)
	S += (t81*MAS[0])-1 < t9_t3_mem1*MAS_MEM[1]
	S += (t81*MAS[1])-1 < t9_t3_mem1*MAS_MEM[3]
	S += (t81*MAS[2])-1 < t9_t3_mem1*MAS_MEM[5]
	S += (t81*MAS[3])-1 < t9_t3_mem1*MAS_MEM[7]
	S += (t81*MAS[4])-1 < t9_t3_mem1*MAS_MEM[9]
	S += (t81*MAS[5])-1 < t9_t3_mem1*MAS_MEM[11]
	S += (t81*MAS[6])-1 < t9_t3_mem1*MAS_MEM[13]
	S += (t81*MAS[7])-1 < t9_t3_mem1*MAS_MEM[15]
	S += t9_t3_mem1 <= t9_t3

	t10_t0 = S.Task('t10_t0', length=3, delay_cost=1)
	t10_t0 += alt(MAS)
	t10_t0_in = S.Task('t10_t0_in', length=1, delay_cost=1)
	t10_t0_in += alt(MAS_in)
	S += t10_t0_in*MAS_in[0]<=t10_t0*MAS[0]

	S += t10_t0_in*MAS_in[1]<=t10_t0*MAS[1]

	S += t10_t0_in*MAS_in[2]<=t10_t0*MAS[2]

	S += t10_t0_in*MAS_in[3]<=t10_t0*MAS[3]

	S += t10_t0_in*MAS_in[4]<=t10_t0*MAS[4]

	S += t10_t0_in*MAS_in[5]<=t10_t0*MAS[5]

	S += t10_t0_in*MAS_in[6]<=t10_t0*MAS[6]

	S += t10_t0_in*MAS_in[7]<=t10_t0*MAS[7]

	t10_t0_mem0 = S.Task('t10_t0_mem0', length=1, delay_cost=1)
	t10_t0_mem0 += alt(MAS_MEM)
	S += (t20*MAS[0])-1 < t10_t0_mem0*MAS_MEM[0]
	S += (t20*MAS[1])-1 < t10_t0_mem0*MAS_MEM[2]
	S += (t20*MAS[2])-1 < t10_t0_mem0*MAS_MEM[4]
	S += (t20*MAS[3])-1 < t10_t0_mem0*MAS_MEM[6]
	S += (t20*MAS[4])-1 < t10_t0_mem0*MAS_MEM[8]
	S += (t20*MAS[5])-1 < t10_t0_mem0*MAS_MEM[10]
	S += (t20*MAS[6])-1 < t10_t0_mem0*MAS_MEM[12]
	S += (t20*MAS[7])-1 < t10_t0_mem0*MAS_MEM[14]
	S += t10_t0_mem0 <= t10_t0

	t10_t0_mem1 = S.Task('t10_t0_mem1', length=1, delay_cost=1)
	t10_t0_mem1 += alt(MAS_MEM)
	S += (t21*MAS[0])-1 < t10_t0_mem1*MAS_MEM[1]
	S += (t21*MAS[1])-1 < t10_t0_mem1*MAS_MEM[3]
	S += (t21*MAS[2])-1 < t10_t0_mem1*MAS_MEM[5]
	S += (t21*MAS[3])-1 < t10_t0_mem1*MAS_MEM[7]
	S += (t21*MAS[4])-1 < t10_t0_mem1*MAS_MEM[9]
	S += (t21*MAS[5])-1 < t10_t0_mem1*MAS_MEM[11]
	S += (t21*MAS[6])-1 < t10_t0_mem1*MAS_MEM[13]
	S += (t21*MAS[7])-1 < t10_t0_mem1*MAS_MEM[15]
	S += t10_t0_mem1 <= t10_t0

	t10_t1 = S.Task('t10_t1', length=3, delay_cost=1)
	t10_t1 += alt(MAS)
	t10_t1_in = S.Task('t10_t1_in', length=1, delay_cost=1)
	t10_t1_in += alt(MAS_in)
	S += t10_t1_in*MAS_in[0]<=t10_t1*MAS[0]

	S += t10_t1_in*MAS_in[1]<=t10_t1*MAS[1]

	S += t10_t1_in*MAS_in[2]<=t10_t1*MAS[2]

	S += t10_t1_in*MAS_in[3]<=t10_t1*MAS[3]

	S += t10_t1_in*MAS_in[4]<=t10_t1*MAS[4]

	S += t10_t1_in*MAS_in[5]<=t10_t1*MAS[5]

	S += t10_t1_in*MAS_in[6]<=t10_t1*MAS[6]

	S += t10_t1_in*MAS_in[7]<=t10_t1*MAS[7]

	t10_t1_mem0 = S.Task('t10_t1_mem0', length=1, delay_cost=1)
	t10_t1_mem0 += alt(MAS_MEM)
	S += (t20*MAS[0])-1 < t10_t1_mem0*MAS_MEM[0]
	S += (t20*MAS[1])-1 < t10_t1_mem0*MAS_MEM[2]
	S += (t20*MAS[2])-1 < t10_t1_mem0*MAS_MEM[4]
	S += (t20*MAS[3])-1 < t10_t1_mem0*MAS_MEM[6]
	S += (t20*MAS[4])-1 < t10_t1_mem0*MAS_MEM[8]
	S += (t20*MAS[5])-1 < t10_t1_mem0*MAS_MEM[10]
	S += (t20*MAS[6])-1 < t10_t1_mem0*MAS_MEM[12]
	S += (t20*MAS[7])-1 < t10_t1_mem0*MAS_MEM[14]
	S += t10_t1_mem0 <= t10_t1

	t10_t1_mem1 = S.Task('t10_t1_mem1', length=1, delay_cost=1)
	t10_t1_mem1 += alt(MAS_MEM)
	S += (t21*MAS[0])-1 < t10_t1_mem1*MAS_MEM[1]
	S += (t21*MAS[1])-1 < t10_t1_mem1*MAS_MEM[3]
	S += (t21*MAS[2])-1 < t10_t1_mem1*MAS_MEM[5]
	S += (t21*MAS[3])-1 < t10_t1_mem1*MAS_MEM[7]
	S += (t21*MAS[4])-1 < t10_t1_mem1*MAS_MEM[9]
	S += (t21*MAS[5])-1 < t10_t1_mem1*MAS_MEM[11]
	S += (t21*MAS[6])-1 < t10_t1_mem1*MAS_MEM[13]
	S += (t21*MAS[7])-1 < t10_t1_mem1*MAS_MEM[15]
	S += t10_t1_mem1 <= t10_t1

	t10_t3 = S.Task('t10_t3', length=11, delay_cost=1)
	t10_t3 += alt(MM)
	t10_t3_in = S.Task('t10_t3_in', length=1, delay_cost=1)
	t10_t3_in += alt(MM_in)
	S += t10_t3_in*MM_in[0]<=t10_t3*MM[0]
	S += t10_t3_in*MM_in[1]<=t10_t3*MM[1]
	t10_t3_mem0 = S.Task('t10_t3_mem0', length=1, delay_cost=1)
	t10_t3_mem0 += alt(MAS_MEM)
	S += (t20*MAS[0])-1 < t10_t3_mem0*MAS_MEM[0]
	S += (t20*MAS[1])-1 < t10_t3_mem0*MAS_MEM[2]
	S += (t20*MAS[2])-1 < t10_t3_mem0*MAS_MEM[4]
	S += (t20*MAS[3])-1 < t10_t3_mem0*MAS_MEM[6]
	S += (t20*MAS[4])-1 < t10_t3_mem0*MAS_MEM[8]
	S += (t20*MAS[5])-1 < t10_t3_mem0*MAS_MEM[10]
	S += (t20*MAS[6])-1 < t10_t3_mem0*MAS_MEM[12]
	S += (t20*MAS[7])-1 < t10_t3_mem0*MAS_MEM[14]
	S += t10_t3_mem0 <= t10_t3

	t10_t3_mem1 = S.Task('t10_t3_mem1', length=1, delay_cost=1)
	t10_t3_mem1 += alt(MAS_MEM)
	S += (t21*MAS[0])-1 < t10_t3_mem1*MAS_MEM[1]
	S += (t21*MAS[1])-1 < t10_t3_mem1*MAS_MEM[3]
	S += (t21*MAS[2])-1 < t10_t3_mem1*MAS_MEM[5]
	S += (t21*MAS[3])-1 < t10_t3_mem1*MAS_MEM[7]
	S += (t21*MAS[4])-1 < t10_t3_mem1*MAS_MEM[9]
	S += (t21*MAS[5])-1 < t10_t3_mem1*MAS_MEM[11]
	S += (t21*MAS[6])-1 < t10_t3_mem1*MAS_MEM[13]
	S += (t21*MAS[7])-1 < t10_t3_mem1*MAS_MEM[15]
	S += t10_t3_mem1 <= t10_t3

	t31 = S.Task('t31', length=3, delay_cost=1)
	t31 += alt(MAS)
	t31_in = S.Task('t31_in', length=1, delay_cost=1)
	t31_in += alt(MAS_in)
	S += t31_in*MAS_in[0]<=t31*MAS[0]

	S += t31_in*MAS_in[1]<=t31*MAS[1]

	S += t31_in*MAS_in[2]<=t31*MAS[2]

	S += t31_in*MAS_in[3]<=t31*MAS[3]

	S += t31_in*MAS_in[4]<=t31*MAS[4]

	S += t31_in*MAS_in[5]<=t31*MAS[5]

	S += t31_in*MAS_in[6]<=t31*MAS[6]

	S += t31_in*MAS_in[7]<=t31*MAS[7]

	t31_mem0 = S.Task('t31_mem0', length=1, delay_cost=1)
	t31_mem0 += alt(MM_MEM)
	S += (t3_t4*MM[0])-1 < t31_mem0*MM_MEM[0]
	S += (t3_t4*MM[1])-1 < t31_mem0*MM_MEM[2]
	S += t31_mem0 <= t31

	t31_mem1 = S.Task('t31_mem1', length=1, delay_cost=1)
	t31_mem1 += alt(MAS_MEM)
	S += (t3_t5*MAS[0])-1 < t31_mem1*MAS_MEM[1]
	S += (t3_t5*MAS[1])-1 < t31_mem1*MAS_MEM[3]
	S += (t3_t5*MAS[2])-1 < t31_mem1*MAS_MEM[5]
	S += (t3_t5*MAS[3])-1 < t31_mem1*MAS_MEM[7]
	S += (t3_t5*MAS[4])-1 < t31_mem1*MAS_MEM[9]
	S += (t3_t5*MAS[5])-1 < t31_mem1*MAS_MEM[11]
	S += (t3_t5*MAS[6])-1 < t31_mem1*MAS_MEM[13]
	S += (t3_t5*MAS[7])-1 < t31_mem1*MAS_MEM[15]
	S += t31_mem1 <= t31

	t110 = S.Task('t110', length=3, delay_cost=1)
	t110 += alt(MAS)
	t110_in = S.Task('t110_in', length=1, delay_cost=1)
	t110_in += alt(MAS_in)
	S += t110_in*MAS_in[0]<=t110*MAS[0]

	S += t110_in*MAS_in[1]<=t110*MAS[1]

	S += t110_in*MAS_in[2]<=t110*MAS[2]

	S += t110_in*MAS_in[3]<=t110*MAS[3]

	S += t110_in*MAS_in[4]<=t110*MAS[4]

	S += t110_in*MAS_in[5]<=t110*MAS[5]

	S += t110_in*MAS_in[6]<=t110*MAS[6]

	S += t110_in*MAS_in[7]<=t110*MAS[7]

	t110_mem0 = S.Task('t110_mem0', length=1, delay_cost=1)
	t110_mem0 += alt(MAS_MEM)
	S += (t30*MAS[0])-1 < t110_mem0*MAS_MEM[0]
	S += (t30*MAS[1])-1 < t110_mem0*MAS_MEM[2]
	S += (t30*MAS[2])-1 < t110_mem0*MAS_MEM[4]
	S += (t30*MAS[3])-1 < t110_mem0*MAS_MEM[6]
	S += (t30*MAS[4])-1 < t110_mem0*MAS_MEM[8]
	S += (t30*MAS[5])-1 < t110_mem0*MAS_MEM[10]
	S += (t30*MAS[6])-1 < t110_mem0*MAS_MEM[12]
	S += (t30*MAS[7])-1 < t110_mem0*MAS_MEM[14]
	S += t110_mem0 <= t110

	t110_mem1 = S.Task('t110_mem1', length=1, delay_cost=1)
	t110_mem1 += alt(MAS_MEM)
	S += (t30*MAS[0])-1 < t110_mem1*MAS_MEM[1]
	S += (t30*MAS[1])-1 < t110_mem1*MAS_MEM[3]
	S += (t30*MAS[2])-1 < t110_mem1*MAS_MEM[5]
	S += (t30*MAS[3])-1 < t110_mem1*MAS_MEM[7]
	S += (t30*MAS[4])-1 < t110_mem1*MAS_MEM[9]
	S += (t30*MAS[5])-1 < t110_mem1*MAS_MEM[11]
	S += (t30*MAS[6])-1 < t110_mem1*MAS_MEM[13]
	S += (t30*MAS[7])-1 < t110_mem1*MAS_MEM[15]
	S += t110_mem1 <= t110

	t40 = S.Task('t40', length=3, delay_cost=1)
	t40 += alt(MAS)
	t40_in = S.Task('t40_in', length=1, delay_cost=1)
	t40_in += alt(MAS_in)
	S += t40_in*MAS_in[0]<=t40*MAS[0]

	S += t40_in*MAS_in[1]<=t40*MAS[1]

	S += t40_in*MAS_in[2]<=t40*MAS[2]

	S += t40_in*MAS_in[3]<=t40*MAS[3]

	S += t40_in*MAS_in[4]<=t40*MAS[4]

	S += t40_in*MAS_in[5]<=t40*MAS[5]

	S += t40_in*MAS_in[6]<=t40*MAS[6]

	S += t40_in*MAS_in[7]<=t40*MAS[7]

	t40_mem0 = S.Task('t40_mem0', length=1, delay_cost=1)
	t40_mem0 += alt(MM_MEM)
	S += (t4_t2*MM[0])-1 < t40_mem0*MM_MEM[0]
	S += (t4_t2*MM[1])-1 < t40_mem0*MM_MEM[2]
	S += t40_mem0 <= t40

	t40_mem1 = S.Task('t40_mem1', length=1, delay_cost=1)
	t40_mem1 += alt(MAS_MEM)
	S += (t4_t5*MAS[0])-1 < t40_mem1*MAS_MEM[1]
	S += (t4_t5*MAS[1])-1 < t40_mem1*MAS_MEM[3]
	S += (t4_t5*MAS[2])-1 < t40_mem1*MAS_MEM[5]
	S += (t4_t5*MAS[3])-1 < t40_mem1*MAS_MEM[7]
	S += (t4_t5*MAS[4])-1 < t40_mem1*MAS_MEM[9]
	S += (t4_t5*MAS[5])-1 < t40_mem1*MAS_MEM[11]
	S += (t4_t5*MAS[6])-1 < t40_mem1*MAS_MEM[13]
	S += (t4_t5*MAS[7])-1 < t40_mem1*MAS_MEM[15]
	S += t40_mem1 <= t40

	t51 = S.Task('t51', length=3, delay_cost=1)
	t51 += alt(MAS)
	t51_in = S.Task('t51_in', length=1, delay_cost=1)
	t51_in += alt(MAS_in)
	S += t51_in*MAS_in[0]<=t51*MAS[0]

	S += t51_in*MAS_in[1]<=t51*MAS[1]

	S += t51_in*MAS_in[2]<=t51*MAS[2]

	S += t51_in*MAS_in[3]<=t51*MAS[3]

	S += t51_in*MAS_in[4]<=t51*MAS[4]

	S += t51_in*MAS_in[5]<=t51*MAS[5]

	S += t51_in*MAS_in[6]<=t51*MAS[6]

	S += t51_in*MAS_in[7]<=t51*MAS[7]

	t51_mem0 = S.Task('t51_mem0', length=1, delay_cost=1)
	t51_mem0 += alt(MM_MEM)
	S += (t5_t4*MM[0])-1 < t51_mem0*MM_MEM[0]
	S += (t5_t4*MM[1])-1 < t51_mem0*MM_MEM[2]
	S += t51_mem0 <= t51

	t51_mem1 = S.Task('t51_mem1', length=1, delay_cost=1)
	t51_mem1 += alt(MAS_MEM)
	S += (t5_t5*MAS[0])-1 < t51_mem1*MAS_MEM[1]
	S += (t5_t5*MAS[1])-1 < t51_mem1*MAS_MEM[3]
	S += (t5_t5*MAS[2])-1 < t51_mem1*MAS_MEM[5]
	S += (t5_t5*MAS[3])-1 < t51_mem1*MAS_MEM[7]
	S += (t5_t5*MAS[4])-1 < t51_mem1*MAS_MEM[9]
	S += (t5_t5*MAS[5])-1 < t51_mem1*MAS_MEM[11]
	S += (t5_t5*MAS[6])-1 < t51_mem1*MAS_MEM[13]
	S += (t5_t5*MAS[7])-1 < t51_mem1*MAS_MEM[15]
	S += t51_mem1 <= t51

	t61 = S.Task('t61', length=3, delay_cost=1)
	t61 += alt(MAS)
	t61_in = S.Task('t61_in', length=1, delay_cost=1)
	t61_in += alt(MAS_in)
	S += t61_in*MAS_in[0]<=t61*MAS[0]

	S += t61_in*MAS_in[1]<=t61*MAS[1]

	S += t61_in*MAS_in[2]<=t61*MAS[2]

	S += t61_in*MAS_in[3]<=t61*MAS[3]

	S += t61_in*MAS_in[4]<=t61*MAS[4]

	S += t61_in*MAS_in[5]<=t61*MAS[5]

	S += t61_in*MAS_in[6]<=t61*MAS[6]

	S += t61_in*MAS_in[7]<=t61*MAS[7]

	t61_mem0 = S.Task('t61_mem0', length=1, delay_cost=1)
	t61_mem0 += alt(MM_MEM)
	S += (t6_t4*MM[0])-1 < t61_mem0*MM_MEM[0]
	S += (t6_t4*MM[1])-1 < t61_mem0*MM_MEM[2]
	S += t61_mem0 <= t61

	t61_mem1 = S.Task('t61_mem1', length=1, delay_cost=1)
	t61_mem1 += alt(MAS_MEM)
	S += (t6_t5*MAS[0])-1 < t61_mem1*MAS_MEM[1]
	S += (t6_t5*MAS[1])-1 < t61_mem1*MAS_MEM[3]
	S += (t6_t5*MAS[2])-1 < t61_mem1*MAS_MEM[5]
	S += (t6_t5*MAS[3])-1 < t61_mem1*MAS_MEM[7]
	S += (t6_t5*MAS[4])-1 < t61_mem1*MAS_MEM[9]
	S += (t6_t5*MAS[5])-1 < t61_mem1*MAS_MEM[11]
	S += (t6_t5*MAS[6])-1 < t61_mem1*MAS_MEM[13]
	S += (t6_t5*MAS[7])-1 < t61_mem1*MAS_MEM[15]
	S += t61_mem1 <= t61

	t71 = S.Task('t71', length=3, delay_cost=1)
	t71 += alt(MAS)
	t71_in = S.Task('t71_in', length=1, delay_cost=1)
	t71_in += alt(MAS_in)
	S += t71_in*MAS_in[0]<=t71*MAS[0]

	S += t71_in*MAS_in[1]<=t71*MAS[1]

	S += t71_in*MAS_in[2]<=t71*MAS[2]

	S += t71_in*MAS_in[3]<=t71*MAS[3]

	S += t71_in*MAS_in[4]<=t71*MAS[4]

	S += t71_in*MAS_in[5]<=t71*MAS[5]

	S += t71_in*MAS_in[6]<=t71*MAS[6]

	S += t71_in*MAS_in[7]<=t71*MAS[7]

	t71_mem0 = S.Task('t71_mem0', length=1, delay_cost=1)
	t71_mem0 += alt(MM_MEM)
	S += (t7_t4*MM[0])-1 < t71_mem0*MM_MEM[0]
	S += (t7_t4*MM[1])-1 < t71_mem0*MM_MEM[2]
	S += t71_mem0 <= t71

	t71_mem1 = S.Task('t71_mem1', length=1, delay_cost=1)
	t71_mem1 += alt(MAS_MEM)
	S += (t7_t5*MAS[0])-1 < t71_mem1*MAS_MEM[1]
	S += (t7_t5*MAS[1])-1 < t71_mem1*MAS_MEM[3]
	S += (t7_t5*MAS[2])-1 < t71_mem1*MAS_MEM[5]
	S += (t7_t5*MAS[3])-1 < t71_mem1*MAS_MEM[7]
	S += (t7_t5*MAS[4])-1 < t71_mem1*MAS_MEM[9]
	S += (t7_t5*MAS[5])-1 < t71_mem1*MAS_MEM[11]
	S += (t7_t5*MAS[6])-1 < t71_mem1*MAS_MEM[13]
	S += (t7_t5*MAS[7])-1 < t71_mem1*MAS_MEM[15]
	S += t71_mem1 <= t71

	t290 = S.Task('t290', length=3, delay_cost=1)
	t290 += alt(MAS)
	t290_in = S.Task('t290_in', length=1, delay_cost=1)
	t290_in += alt(MAS_in)
	S += t290_in*MAS_in[0]<=t290*MAS[0]

	S += t290_in*MAS_in[1]<=t290*MAS[1]

	S += t290_in*MAS_in[2]<=t290*MAS[2]

	S += t290_in*MAS_in[3]<=t290*MAS[3]

	S += t290_in*MAS_in[4]<=t290*MAS[4]

	S += t290_in*MAS_in[5]<=t290*MAS[5]

	S += t290_in*MAS_in[6]<=t290*MAS[6]

	S += t290_in*MAS_in[7]<=t290*MAS[7]

	t290_mem0 = S.Task('t290_mem0', length=1, delay_cost=1)
	t290_mem0 += alt(MAS_MEM)
	S += (t50*MAS[0])-1 < t290_mem0*MAS_MEM[0]
	S += (t50*MAS[1])-1 < t290_mem0*MAS_MEM[2]
	S += (t50*MAS[2])-1 < t290_mem0*MAS_MEM[4]
	S += (t50*MAS[3])-1 < t290_mem0*MAS_MEM[6]
	S += (t50*MAS[4])-1 < t290_mem0*MAS_MEM[8]
	S += (t50*MAS[5])-1 < t290_mem0*MAS_MEM[10]
	S += (t50*MAS[6])-1 < t290_mem0*MAS_MEM[12]
	S += (t50*MAS[7])-1 < t290_mem0*MAS_MEM[14]
	S += t290_mem0 <= t290

	t290_mem1 = S.Task('t290_mem1', length=1, delay_cost=1)
	t290_mem1 += alt(MAS_MEM)
	S += (t50*MAS[0])-1 < t290_mem1*MAS_MEM[1]
	S += (t50*MAS[1])-1 < t290_mem1*MAS_MEM[3]
	S += (t50*MAS[2])-1 < t290_mem1*MAS_MEM[5]
	S += (t50*MAS[3])-1 < t290_mem1*MAS_MEM[7]
	S += (t50*MAS[4])-1 < t290_mem1*MAS_MEM[9]
	S += (t50*MAS[5])-1 < t290_mem1*MAS_MEM[11]
	S += (t50*MAS[6])-1 < t290_mem1*MAS_MEM[13]
	S += (t50*MAS[7])-1 < t290_mem1*MAS_MEM[15]
	S += t290_mem1 <= t290

	t151 = S.Task('t151', length=3, delay_cost=1)
	t151 += alt(MAS)
	t151_in = S.Task('t151_in', length=1, delay_cost=1)
	t151_in += alt(MAS_in)
	S += t151_in*MAS_in[0]<=t151*MAS[0]

	S += t151_in*MAS_in[1]<=t151*MAS[1]

	S += t151_in*MAS_in[2]<=t151*MAS[2]

	S += t151_in*MAS_in[3]<=t151*MAS[3]

	S += t151_in*MAS_in[4]<=t151*MAS[4]

	S += t151_in*MAS_in[5]<=t151*MAS[5]

	S += t151_in*MAS_in[6]<=t151*MAS[6]

	S += t151_in*MAS_in[7]<=t151*MAS[7]

	t151_mem0 = S.Task('t151_mem0', length=1, delay_cost=1)
	t151_mem0 += alt(MAS_MEM)
	S += (t01*MAS[0])-1 < t151_mem0*MAS_MEM[0]
	S += (t01*MAS[1])-1 < t151_mem0*MAS_MEM[2]
	S += (t01*MAS[2])-1 < t151_mem0*MAS_MEM[4]
	S += (t01*MAS[3])-1 < t151_mem0*MAS_MEM[6]
	S += (t01*MAS[4])-1 < t151_mem0*MAS_MEM[8]
	S += (t01*MAS[5])-1 < t151_mem0*MAS_MEM[10]
	S += (t01*MAS[6])-1 < t151_mem0*MAS_MEM[12]
	S += (t01*MAS[7])-1 < t151_mem0*MAS_MEM[14]
	S += t151_mem0 <= t151

	t151_mem1 = S.Task('t151_mem1', length=1, delay_cost=1)
	t151_mem1 += alt(MAS_MEM)
	S += (t41*MAS[0])-1 < t151_mem1*MAS_MEM[1]
	S += (t41*MAS[1])-1 < t151_mem1*MAS_MEM[3]
	S += (t41*MAS[2])-1 < t151_mem1*MAS_MEM[5]
	S += (t41*MAS[3])-1 < t151_mem1*MAS_MEM[7]
	S += (t41*MAS[4])-1 < t151_mem1*MAS_MEM[9]
	S += (t41*MAS[5])-1 < t151_mem1*MAS_MEM[11]
	S += (t41*MAS[6])-1 < t151_mem1*MAS_MEM[13]
	S += (t41*MAS[7])-1 < t151_mem1*MAS_MEM[15]
	S += t151_mem1 <= t151

	t220 = S.Task('t220', length=3, delay_cost=1)
	t220 += alt(MAS)
	t220_in = S.Task('t220_in', length=1, delay_cost=1)
	t220_in += alt(MAS_in)
	S += t220_in*MAS_in[0]<=t220*MAS[0]

	S += t220_in*MAS_in[1]<=t220*MAS[1]

	S += t220_in*MAS_in[2]<=t220*MAS[2]

	S += t220_in*MAS_in[3]<=t220*MAS[3]

	S += t220_in*MAS_in[4]<=t220*MAS[4]

	S += t220_in*MAS_in[5]<=t220*MAS[5]

	S += t220_in*MAS_in[6]<=t220*MAS[6]

	S += t220_in*MAS_in[7]<=t220*MAS[7]

	t220_mem0 = S.Task('t220_mem0', length=1, delay_cost=1)
	t220_mem0 += alt(MAS_MEM)
	S += (t60*MAS[0])-1 < t220_mem0*MAS_MEM[0]
	S += (t60*MAS[1])-1 < t220_mem0*MAS_MEM[2]
	S += (t60*MAS[2])-1 < t220_mem0*MAS_MEM[4]
	S += (t60*MAS[3])-1 < t220_mem0*MAS_MEM[6]
	S += (t60*MAS[4])-1 < t220_mem0*MAS_MEM[8]
	S += (t60*MAS[5])-1 < t220_mem0*MAS_MEM[10]
	S += (t60*MAS[6])-1 < t220_mem0*MAS_MEM[12]
	S += (t60*MAS[7])-1 < t220_mem0*MAS_MEM[14]
	S += t220_mem0 <= t220

	t220_mem1 = S.Task('t220_mem1', length=1, delay_cost=1)
	t220_mem1 += alt(MAS_MEM)
	S += (t60*MAS[0])-1 < t220_mem1*MAS_MEM[1]
	S += (t60*MAS[1])-1 < t220_mem1*MAS_MEM[3]
	S += (t60*MAS[2])-1 < t220_mem1*MAS_MEM[5]
	S += (t60*MAS[3])-1 < t220_mem1*MAS_MEM[7]
	S += (t60*MAS[4])-1 < t220_mem1*MAS_MEM[9]
	S += (t60*MAS[5])-1 < t220_mem1*MAS_MEM[11]
	S += (t60*MAS[6])-1 < t220_mem1*MAS_MEM[13]
	S += (t60*MAS[7])-1 < t220_mem1*MAS_MEM[15]
	S += t220_mem1 <= t220

	t260 = S.Task('t260', length=3, delay_cost=1)
	t260 += alt(MAS)
	t260_in = S.Task('t260_in', length=1, delay_cost=1)
	t260_in += alt(MAS_in)
	S += t260_in*MAS_in[0]<=t260*MAS[0]

	S += t260_in*MAS_in[1]<=t260*MAS[1]

	S += t260_in*MAS_in[2]<=t260*MAS[2]

	S += t260_in*MAS_in[3]<=t260*MAS[3]

	S += t260_in*MAS_in[4]<=t260*MAS[4]

	S += t260_in*MAS_in[5]<=t260*MAS[5]

	S += t260_in*MAS_in[6]<=t260*MAS[6]

	S += t260_in*MAS_in[7]<=t260*MAS[7]

	t260_mem0 = S.Task('t260_mem0', length=1, delay_cost=1)
	t260_mem0 += alt(MAS_MEM)
	S += (t70*MAS[0])-1 < t260_mem0*MAS_MEM[0]
	S += (t70*MAS[1])-1 < t260_mem0*MAS_MEM[2]
	S += (t70*MAS[2])-1 < t260_mem0*MAS_MEM[4]
	S += (t70*MAS[3])-1 < t260_mem0*MAS_MEM[6]
	S += (t70*MAS[4])-1 < t260_mem0*MAS_MEM[8]
	S += (t70*MAS[5])-1 < t260_mem0*MAS_MEM[10]
	S += (t70*MAS[6])-1 < t260_mem0*MAS_MEM[12]
	S += (t70*MAS[7])-1 < t260_mem0*MAS_MEM[14]
	S += t260_mem0 <= t260

	t260_mem1 = S.Task('t260_mem1', length=1, delay_cost=1)
	t260_mem1 += alt(MAS_MEM)
	S += (t70*MAS[0])-1 < t260_mem1*MAS_MEM[1]
	S += (t70*MAS[1])-1 < t260_mem1*MAS_MEM[3]
	S += (t70*MAS[2])-1 < t260_mem1*MAS_MEM[5]
	S += (t70*MAS[3])-1 < t260_mem1*MAS_MEM[7]
	S += (t70*MAS[4])-1 < t260_mem1*MAS_MEM[9]
	S += (t70*MAS[5])-1 < t260_mem1*MAS_MEM[11]
	S += (t70*MAS[6])-1 < t260_mem1*MAS_MEM[13]
	S += (t70*MAS[7])-1 < t260_mem1*MAS_MEM[15]
	S += t260_mem1 <= t260

	solvers.mip.solve(S,msg=1,kind='CPLEX',ratio_gap=1.01)

	solution = [['hoge']*len(S.solution()[1]) for i in range(len(S.solution()))]
	for i in range(len(S.solution())):
		for j in range(len(S.solution()[i])):
			solution[i][j]=str(S.solution()[i][j])
	print(solution)

	cycles = int(solution[-1][3])

	pic_file_name = "/mnt/rose/usr1/fukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage11MM2_stage3MAS8/SQR012345/schedule1.png"
	if(S.solution() != []):
		plotters.matplotlib.plot(S,img_filename=pic_file_name, show_task_labels=False, fig_size=(cycles*0.25+3, 10))

	return solution

