...
def solve():
	...
	c10_t1 = S.Task('c10_t1', length=9, delay_cost=1)
	S += c10_t1 >= 5
	c10_t1 += MM[0]

	c01_t1 = S.Task('c01_t1', length=9, delay_cost=1)
	S += c01_t1 >= 6
	c01_t1 += MM[0]
	...
	c20_t0 = S.Task('c20_t0', length=9, delay_cost=1)
	S += c20_t0 >= 7
	c20_t0 += MM[1]
	...
	c21_t1 = S.Task('c21_t1', length=9, delay_cost=1)
	S += c21_t1 >= 8
	c21_t1 += MM[0]

	c11_t1 = S.Task('c11_t1', length=9, delay_cost=1)
	S += c11_t1 >= 9
	c11_t1 += MM[1]
	...
	c20_t1 = S.Task('c20_t1', length=9, delay_cost=1)
	S += c20_t1 >= 10
	c20_t1 += MM[0]
	...
	c10_t0 = S.Task('c10_t0', length=9, delay_cost=1)
	S += c10_t0 >= 15
	c10_t0 += MM[0]
	...
	c01_t0 = S.Task('c01_t0', length=9, delay_cost=1)
	S += c01_t0 >= 19
	c01_t0 += MM[0]
	...
	c11_t0 = S.Task('c11_t0', length=9, delay_cost=1)
	S += c11_t0 >= 20
	c11_t0 += MM[0]
	...
	c21_t0 = S.Task('c21_t0', length=9, delay_cost=1)
	S += c21_t0 >= 21
	c21_t0 += MM[0]
	...
	# new tasks
	c10_t4 = S.Task('c10_t4', length=9, delay_cost=1)
	c10_t4 += alt(MM)
	...
	c20_t4 = S.Task('c20_t4', length=9, delay_cost=1)
	c20_t4 += alt(MM)
	...
	c01_t4 = S.Task('c01_t4', length=9, delay_cost=1)
	c01_t4 += alt(MM)
	...
	c11_t4 = S.Task('c11_t4', length=9, delay_cost=1)
	c11_t4 += alt(MM)
	...
	c21_t4 = S.Task('c21_t4', length=9, delay_cost=1)
	c21_t4 += alt(MM)
	...
	pic_file_name = "/mnt/rose/usr1/fukuda/ABE/multiRAM_multiMAS_16mux/scheduling_result/stage9MM2_stage2MAS6/FROB/schedule1.png"
	if(S.solution() != []):
